...
SEC EDGAR filings/insider trades/company facts, US Treasury yield curve,
...
- Yahoo Finance API: https://query1.finance.yahoo.com (stock indices, ETF, futures, history, options, no auth)
- SEC EDGAR: https://data.sec.gov (filings, insider trades, XBRL facts, no auth)
- US Treasury: https://home.treasury.gov (yield curve, historical yields, no auth)
"""
...
def _ticker_to_cik(ticker: str) -> str | None:
    ...
    r = httpx.get(url, headers=_edgar_headers(), timeout=10)
    r.raise_for_status()
    data = r.json()
...
@mcp.tool()
def get_sec_filings(ticker: str, form_type: str = "10-K", limit: int = 5) -> dict:
    """
...
def get_insider_trades(ticker: str, limit: int = 10) -> dict:
    ...
    try:
        ...
        dates = filings.get("filingDate", [])
        accessions = filings.get("accessionNumber", [])
        reporters = filings.get("reportingOwner", []) if "reportingOwner" in filings else [""] * len(forms)
        results = []
        for i, form in enumerate(forms):
...
@mcp.tool()
def get_company_facts(ticker: str, concept: str = "us-gaap/Revenues") -> dict:
    """
    ...
    try:
        ...
        r = httpx.get(url, headers=_edgar_headers(), timeout=15)
        r.raise_for_status()
        data = r.json()
...
@mcp.tool()
def get_treasury_yield_curve() -> dict:
    """
...
async def get_options_chain(ticker: str, expiry: Optional[str] = None) -> dict:
    ...
    Returns:
        ...
        implied volatility, and in-the-money status. Top 10 by open interest each side.
    """
    try:
        tk = yf.Ticker(ticker)
        ...
        def _fmt(df):
            ...
            for _, r in df.iterrows():
                rows.append({
                    ...
                    "volume": int(r.get("volume", 0)) if str(r.get("volume", "")) not in ["nan", "None"] else 0,
                    "openInterest": int(r.get("openInterest", 0)) if str(r.get("openInterest", "")) not in ["nan", "None"] else 0,
                    "impliedVolatility": round(float(r.get("impliedVolatility", 0)), 4),
                    "inTheMoney": bool(r.get("inTheMoney", False)),
                })
            return rows

        calls = _fmt(chain.calls)
        puts = _fmt(chain.puts)
...
async def get_institutional_holdings(ticker: str) -> dict:
    """
    Institutional holdings (13F filings) for a stock — top fund holders and major holders summary.
    No API key required (Yahoo Finance aggregated 13F data).

    Args:
        ticker: Stock ticker symbol (e.g., 'AAPL', 'MSFT', 'NVDA')

    Returns:
        Top 10 institutional holders, top 5 mutual fund holders,
        major holders summary (% held by insiders/institutions).
    """
    try:
        tk = yf.Ticker(ticker)
...
@mcp.tool()
async def get_historical_treasury_yields(
    maturity: str = "10Y",
    start: Optional[str] = None,
    end: Optional[str] = None,
) -> dict:
    """
    Historical US Treasury yield data for a specific maturity over a date range.
    No API key required (US Treasury XML feed).

    Args:
        maturity: Maturity: '1M', '2M', '3M', '6M', '1Y', '2Y', '3Y', '5Y', '7Y', '10Y', '20Y', '30Y'
    ...
    try:
        ...
        return {
            ...
            "data": records,
            "stats": {
                "min": round(min(yields), 3),
                "max": round(max(yields), 3),
                "mean": round(sum(yields) / len(yields), 3),
                "latest": yields[-1],
            },
            "source": "US Treasury (home.treasury.gov, free)",
        }
...
async def get_economic_calendar(year: int = None) -> dict:
    """
    FOMC meeting schedule for the given year from the Federal Reserve website.
    No API key required (public data).

    Args:
        year: Calendar year (default: current year). Range: 2021-2027.

    Returns:
        List of FOMC meeting dates with statement/minutes status and next upcoming meeting.
    """
    try:
        from datetime import datetime
        if year is None:
        ...
        end_pos = text.find(next_year_marker)
        end_pos = end_pos if end_pos > start else start + 8000
        section = text[start:end_pos] if start != -1 else ""
        ...
        for m in month_names:
            ...
            try:
                ...
                date_iso = datetime(year, month_num, end_day).strftime("%Y-%m-%d")
            except Exception:
                pass
        ...
        for mtg in meetings:
            try:
                if datetime.strptime(mtg["end_date_iso"], "%Y-%m-%d") >= now:
                    upcoming = mtg
                    break
            except Exception:
                pass
        ...
        return {
            ...
            "note": "Meetings with includes_sep=true include Summary of Economic Projections (dot plot).",
...
async def get_crypto_defi_tvl(protocol: str = None) -> dict:
    ...
    Args:
        protocol: Protocol slug (e.g., 'uniswap', 'aave', 'lido', 'makerdao').
                  Use None or 'top' to get the top 20 DeFi protocols by TVL.

    Returns:
        Current TVL, 24h/7d change, category, chain breakdown, and 30-day TVL history.
    """
    try:
        ...
        if protocol is None or protocol.strip().lower() in ("top", "all", ""):
            ...
            if proto_r.status_code != 200:
                return {"error": f"Protocol '{slug}' not found on DefiLlama. Try exact slug (e.g., 'uniswap', 'aave-v3', 'lido')."}

            d = proto_r.json()
            chain_tvls = {}
            for chain_name, chain_data in (d.get("chainTvls") or {}).items():
                if isinstance(chain_data, dict) and "tvl" in chain_data:
                    series = chain_data["tvl"]
                    if series:
                        chain_tvls[chain_name] = round(series[-1].get("totalLiquidityUSD", 0))
            ...
            if isinstance(raw_tvl, list) and raw_tvl:
                for entry in raw_tvl[-30:]:
                    if isinstance(entry, dict):
                        ts = entry.get("date", 0)
                        try:
                            date_str = _dt.utcfromtimestamp(ts).strftime("%Y-%m-%d")
                        except Exception:
                            date_str = str(ts)
                        tvl_history.append({
                            "date": date_str,
...
# ─────────────────────────────────────────────────────────────────────────────
# v1.5.0 tools (pre-implementation)
# ─────────────────────────────────────────────────────────────────────────────
...
async def get_macro_dashboard() -> dict:
    ...
    try:
        ...
        SYMBOLS = {
            ...
            "sp500": "^GSPC",
            "t10y": "^TNX",
            "t2y": "^IRX",   # 13-week T-bill (closest available proxy for short end)
        }
        ...
        # Fear & Greed (free API, already used elsewhere)
        try:
...
async def get_insider_sentiment(ticker: str) -> dict:
    ...
    try:
        ...
        # Sentiment score: buy_value - sell_value normalized
        ...
        if total_value > 0:
            score = net_value / total_value  # -1 to +1
            if score > 0.3:
                sentiment = "bullish"
            elif score < -0.3:
                sentiment = "bearish"
        ...
        # Top 3 largest transactions (by value)
...
async def get_stock_screener(
    ...
    try:
        ...
        return {
            ...
            "note": "Universe: 80 curated S&P 500 names across all 11 sectors. For full 500-stock screen, reduce universe overlap.",
        }
    except Exception as e:
        return {"error": str(e)}
...
@mcp.tool()
def get_economic_surprise_index(country: str = "US") -> dict:
...
@mcp.tool()
def get_sector_momentum(period_days: int = 20) -> dict:
...
def get_market_breadth() -> dict:
    ...
    try:
        ...
        # 52W High / Low (rolling 252-day window, proxy with available history)
...
@mcp.tool()
def get_volatility_surface(ticker: str) -> dict:
    ...
    try:
        ...
        for exp in expiries[:8]:  # cap at 8 expiries to stay within RAM
            try:
                chain = t.option_chain(exp)
        ...
        # Term structure: short (<30d), mid (30-90d), long (>90d)
        from datetime import datetime
...
@mcp.tool()
def get_fx_volatility(pair: str = "EURUSD") -> dict:
    """
    ...
    try:
        data = yf.download(symbol, period="2y", auto_adjust=True, progress=False)
        if data.empty:
...
def get_credit_spreads() -> dict:
    ...
    try:
        ...
        data = yf.download(symbols, period="1y", auto_adjust=True, progress=False)["Close"]

        results = {}
        for symbol in symbols:
            col = symbol if symbol in data.columns else None
            if col is None:
                continue
        ...
        # Spread proxy: IEF return - credit ETF return
        # Positive = credit underperformed Treasury = spreads widening
        spread_proxies = {}
...
@mcp.tool()
def get_global_equity_heatmap() -> dict:
    """
    ...
    try:
        ...
        # 20 major country index ETFs / proxy tickers (Yahoo Finance)
        # Format: {symbol: (country_name, market_type)}
        country_indices = {
            # Developed Markets
        ...
        # Download ~40 trading days of data (enough for 1M return)
        import yfinance as yf
        raw = yf.download(symbols, period="3mo", auto_adjust=True, progress=False)
        ...
        for sym, (country, market_type) in country_indices.items():
            ...
            # 1D = yesterday close vs 2 days ago close (latest available)
            ret_1d  = safe_ret(1)
            ret_1w  = safe_ret(5)
            ret_1m  = safe_ret(21)
        ...
        # Build heatmap rankings per period
        def rank_period(key):
            scored = [
...
@mcp.tool()
def get_commodity_correlation(period_days: int = 90) -> dict:
    """
    ...
    try:
        ...
        # Commodity tickers (Yahoo Finance futures / ETF proxies)
        commodities = {
            "GC=F":  "Gold",
            "SI=F":  "Silver",
            "CL=F":  "WTI Oil",
        ...
        raw = yf.download(all_syms, period="2y", auto_adjust=True, progress=False)
        ...
        for sym in all_syms:
            ...
            if len(prices) < n_days + 5:
                continue
            # Use last n_days+1 prices for n_days returns
        ...
        # Commodity vs benchmarks
        vs_market = {}
        for cs in comm_syms:
            vs_market[commodities[cs]] = {}
            for bs in bench_syms:
                bench_label = benchmarks[bs].split(" (")[0]  # "S&P 500"
                vs_market[commodities[cs]][bench_label] = fmt(corr.loc[cs, bs])

        # Regime signal based on correlations
        # Gold vs DXY: usually negative. Gold vs SPY: risk barometer.
        gold_spy = None
        oil_spy = None
        gold_dxy = None

        if "GC=F" in df.columns and "SPY" in df.columns:
            gold_spy = fmt(corr.loc["GC=F", "SPY"])
        if "CL=F" in df.columns and "SPY" in df.columns:
            oil_spy = fmt(corr.loc["CL=F", "SPY"])
        if "GC=F" in df.columns and "DX-Y.NYB" in df.columns:
            gold_dxy = fmt(corr.loc["GC=F", "DX-Y.NYB"])
        ...
        # Recent 1M performance of each commodity
        perf = {}
...
def get_earnings_whisper(ticker: str) -> dict:
    ...
    try:
        ...
        # Filter to rows with actual EPS data (not future estimates)
        ...
        # Get price history for reaction analysis
        ...
        for date, row in recent.iterrows():
            ...
            if surprise_pct >= 2:
                ...
                beat_count += 1  # in-line counts as not a miss

            total_surprise_pct += surprise_pct
            bias_pct_list.append(surprise_pct)

            # Price reaction: find trading day of earnings date + next day
            ...
            if not price_hist.empty:
                try:
                    ...
                    earn_date = date.date()
                    # Find index of earnings date or nearest after
        ...
        # Whisper gap: if company consistently beats by X%, market prices in that gap
        # Positive = company usually beats consensus (bullish tendency)
        # Negative = company usually misses (bearish tendency)
        if avg_surprise > 5:
        ...
        # Average price reaction on beats vs misses
...
@mcp.tool()
def get_options_skew_monitor() -> dict:
    """
    ...
    and a simplified 52-week percentile estimate using VIX as baseline.
    ...
    try:
        ...
        for sym, name in indices.items():
            try:
                ...
                # Get nearest expiry options (front month, at least 7 days out)
                expirations = t.options
                if not expirations:
                    results[sym] = {"error": "No options data"}
                    continue

                # Pick expiry 2-6 weeks out for meaningful skew
                from datetime import datetime, timedelta
                ...
                atm_strike = float(atm_call["strike"].iloc[0]) if not atm_call.empty else current_price
                ...
                # OTM call: ~5% above current price
                ...
                # Skew = OTM put IV - OTM call IV (25-delta proxy)
                skew = None
                if otm_put_iv is not None and otm_call_iv is not None:
                    skew = round((otm_put_iv - otm_call_iv) * 100, 2)  # in percentage points
                    overall_skew_scores.append(skew)

                # Put/Call ATM IV ratio
                pc_ratio = None
        ...
        # Composite skew signal across all 3 indices
...
@mcp.tool()
def get_yield_curve_dynamics() -> dict:
    """
...
@mcp.tool()
def get_options_term_structure(ticker: str) -> dict:
    ...
    try:
        ...
        # Bucket into standard tenors
        buckets = {
            "30d": None, "60d": None, "90d": None,
            "120d": None, "180d": None, "360d": None,
        }
        ...
        slope = round(far_iv - near_iv, 2)  # positive = contango, negative = backwardation
        ...
        # Realized vol (30d) for vol risk premium
        rv_30d = None
        vrp_signal = None
        try:
...
@mcp.tool()
def get_earnings_season_tracker() -> dict:
    ...
    try:
        ...
        # Representative S&P 500 proxy by sector (60 tickers, 5-6 per sector)
        sector_tickers = {
        ...
        # Historical S&P 500 beat rate benchmark: ~74% average
        if beat_rate >= 78:
        ...
        # Estimated S&P 500 season progress (proxy: ~60 tickers represents ~12% of S&P 500)
        # In reality, we track whether our sample has data; this approximates season progress
        season_coverage_pct = round(total / len(all_tickers) * 100, 1)

        now = datetime.now()
        # Determine earnings season quarter context
        month = now.month
        if month in [1, 2]:
...
@mcp.tool()
def get_macro_regime_monitor() -> dict:
    """
    ...
    try:
        ...
        # ── 1. GROWTH AXIS ── PMI proxy: ISM Manufacturing proxy via SPY 3M momentum
        # Proxy: if S&P 500 3M return > 5% → expansion, < -5% → contraction
        ...
        # ── 2. INFLATION AXIS ── CPI proxy: Gold (GLD) + Energy (USO) 3M momentum
        infl_tickers = {"gold": "GLD", "energy": "USO", "tips_spread": "TIP"}
        infl_data = {}
        ...
        # ── 3. RATES AXIS ── Yield curve: 10Y - 2Y spread
        ...
        if len(t10) >= 5 and len(t2_proxy) >= 5:
            ...
            # 2Y proxy: interpolate between 3M and 10Y (rough)
        ...
        # ── 4. LIQUIDITY AXIS ── Proxy: Dollar index (DXY) inverse + credit spreads
        ...
        if dxy_3m is not None and lqd_3m is not None:
            # Rising DXY + falling credit = tighter liquidity
        ...
        # ── 5. REGIME MATRIX ──
        # GOLDILOCKS: Expansion + Stable/Disinflationary + Normal/Steep + Neutral/Abundant
        # OVERHEATING: Expansion + High Inflation + Steep + Tight (rate hike cycle)
        # STAGFLATION: Contraction/Neutral + High Inflation → worst case
        # RECESSION: Contraction + Disinflationary + Inverted + Tight
        ...
        liq_easy = liquidity_signal in ("ABUNDANT", "NEUTRAL")
...
@mcp.tool()
def get_dividend_calendar(sector: str = "ALL") -> dict:
    """
    ...
    try:
        ...
        # Major dividend payers by sector (80 stocks)
        sector_tickers_map = {
        ...
        if sector.upper() == "ALL":
            ...
            if not matched:
                return {
                    ...
                    "valid_sectors": ["ALL"] + valid_sectors,
                }
        ...
        def fetch_dividend_info(ticker, sec):
            try:
                ...
                forward_annual_div = info.get("dividendRate", None)
                ex_div_date_ts = info.get("exDividendDate", None)
                payout_ratio = info.get("payoutRatio", None)
                five_yr_avg_div_yield = info.get("fiveYearAvgDividendYield", None)
                trailing_annual_div_rate = info.get("trailingAnnualDividendRate", None)
                trailing_annual_div_yield = info.get("trailingAnnualDividendYield", None)
                price = info.get("currentPrice") or info.get("regularMarketPrice")
                ...
                # Dividend growth proxy: forward vs trailing
                div_growth_yoy = None
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(fetch_dividend_info, tk, sec): tk for tk, sec in tickers_to_check.items()}
        ...
        # Sort: upcoming ex-div first (within 60 days), then by yield desc
        ...
        growing = [r for r in results if r["dividend_growth_yoy_pct"] and r["dividend_growth_yoy_pct"] > 3]
        growing.sort(key=lambda x: x["dividend_growth_yoy_pct"], reverse=True)
...
@mcp.tool()
def get_vix_regime_monitor() -> dict:
    """
    ...
    try:
        ...
        # Fetch VIX and term structure proxy (VXZ = iPath S&P 500 VIX Mid-Term Futures ETN ~5M tenor)
        end = datetime.now()
        ...
        # Term structure proxy: spot VIX vs VXZ (mid-term ~5M futures)
        term_structure_regime = "UNAVAILABLE"
        vxz_current = None
        if not vxz_data.empty:
            vxz_current = round(float(vxz_data[close_col].dropna().iloc[-1]), 2)
            # VIX < VXZ → CONTANGO (normal, vol expected to rise), VIX > VXZ → BACKWARDATION (stressed)
            if current_vix < vxz_current * 0.95:
                term_structure_regime = "CONTANGO"  # normal, calm
            elif current_vix > vxz_current * 1.05:
                term_structure_regime = "BACKWARDATION"  # stressed, vol spike
            else:
...
@mcp.tool()
def get_insider_trading_radar(sector: str = "ALL") -> dict:
    """
    ...
    try:
        ...
        # S&P 500 coverage by sector (key dividend + widely followed names)
        SECTOR_TICKERS = {
            ...
            "Consumer Discretionary": ["AMZN", "TSLA", "HD", "MCD", "NKE", "LOW", "SBUX", "TJX", "BKNG", "CMG", "DHI", "LEN", "PHM", "GM", "F"],
        ...
        if sector == "ALL":
            tickers_by_sector = SECTOR_TICKERS
        else:
            tickers_by_sector = {sector: SECTOR_TICKERS[sector]}
        ...
        def fetch_insider_data(ticker, sec_name):
            try:
                ...
                # The 'Start Date' column contains transaction dates
                date_col = None
                ...
                for _, row in recent.iterrows():
                    ...
                    # Determine direction from text or shares sign
                    try:
                ...
                signal = "STRONG_ACCUMULATION" if buy_count >= 3 and net_value > 500000 else \
                         "ACCUMULATION" if buy_count >= 2 and net_value > 0 else \
                         "SINGLE_BUY" if buy_count >= 1 else "NEUTRAL"
        ...
        # Sort by signal strength and buy count
        signal_rank = {"STRONG_ACCUMULATION": 0, "ACCUMULATION": 1, "SINGLE_BUY": 2, "NEUTRAL": 3}
        ...
        total_buy_value = sum(r["total_buy_value_usd"] for r in results)
        ...
        return {
            ...
            "total_insider_buy_value_usd": round(total_buy_value),
...
def get_etf_flow_tracker() -> dict:
    ...
    try:
        ...
        data = yf.download(tickers_str, period="45d", interval="1d", progress=False, auto_adjust=True)
...
@mcp.tool()
def get_options_gamma_exposure(ticker: str = "SPY") -> dict:
    ...
    try:
        ...
        r_rate = 0.05  # risk-free rate proxy
        ...
        # Find GEX flip point (price where cumulative strike GEX crosses zero)
...
@mcp.tool()
def get_earnings_revision_tracker(ticker: str = "AAPL") -> dict:
    """
...
@mcp.tool()
def get_short_squeeze_radar() -> dict:
    ...
    def fetch_squeeze_data(ticker):
        try:
            ...
            # Squeeze score: short % (0-40), 1W momentum (0-30), volume surge (0-30)
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        results = list(ex.map(fetch_squeeze_data, CANDIDATES))
...
@mcp.tool()
def get_put_call_ratio_history(ticker: str = "SPY", period_days: int = 30) -> dict:
...
@mcp.tool()
def get_institutional_flow_tracker(sector: str = "ALL") -> dict:
    """
    ...
    if sector_upper != "ALL" and sector_upper.title().replace("_", "_") not in SECTOR_UNIVERSE:
    ...
    def analyze_institutional(ticker):
        try:
            ...
            # Institutional holders list — count how many and total shares
            ...
            # Price momentum proxy for flow direction
            ...
            # Volume trend (recent 10d vs previous 10d) as flow proxy
            vol_signal = "NEUTRAL"
            vol_ratio = None
            ...
            # Flow signal heuristic: high inst% + price momentum + vol surge = accumulation
            flow_signal = "NEUTRAL"
...
@mcp.tool()
def get_dark_pool_indicator(ticker: str = "SPY") -> dict:
    """
    ...
    try:
        ...
        # ── 1. OBV (On-Balance Volume) calculation ──
        obv = [0.0]
        ...
        # OBV divergence: price falling but OBV rising = bullish divergence (hidden buying)
        obv_divergence = "NONE"
        if price_slope < -0.02 and obv_slope > 0.01:
            obv_divergence = "BULLISH_DIVERGENCE"  # price down, OBV up = dark pool buying
        elif price_slope > 0.02 and obv_slope < -0.01:
            obv_divergence = "BEARISH_DIVERGENCE"  # price up, OBV down = dark pool selling

        # ── 2. VWAP Proxy ──
        # True VWAP = sum(price×vol) / sum(vol) — 20-day rolling
        ...
        # Close vs VWAP: buying above VWAP = institutional accumulation
        ...
        # ── 3. Block Trade Proxy ──
        # Days where volume > 2× 30d avg AND price range narrow (close within 0.5% of VWAP proxy)
        ...
        for i in range(-10, 0):
            ...
            if vol_ratio >= 1.8 and day_range_pct <= 1.5:
                # High volume, tight range = institutional block trading at a target price
        ...
        # ── 4. Accumulation/Distribution (A/D Line proxy) ──
        ad_values = []
        ad = 0.0
        ...
        return {
            ...
            "block_trade_analysis": {
                ...
                "criteria": "High volume (≥1.8× 30d avg) + tight range (≤1.5% H-L) = potential block trade",
            },
...
@mcp.tool()
def get_options_unusual_activity_scanner(min_volume_oi_ratio: float = 3.0, top_n: int = 20) -> dict:
    """
    ...
    def scan_ticker(tkr):
        try:
            ...
            if not opts:
                return None
            # pick nearest expiration (7-45 DTE)
            import datetime
    ...
    try:
        all_unusual = []
        with ThreadPoolExecutor(max_workers=12) as executor:
            futures = {executor.submit(scan_ticker, tkr): tkr for tkr in UNIVERSE}
...
@mcp.tool()
def get_smart_money_composite(ticker: str = "AAPL") -> dict:
    """
    ...
    try:
        ...
        # ── 2. Institutional Ownership Trend ── max 20 pts ──
        inst_score = 0
        inst_signal = "N/A"
        try:
            info_full = t.info
        ...
        # ── 4. Short Interest (Squeeze Risk as contrarian signal) ── max 15 pts ──
        short_score = 0
        short_signal = "N/A"
        try:
            info_full2 = t.info
            ...
            if short_pct is not None:
                ...
                # low short = bullish (no squeeze risk = healthy)
                # very high short = potential squeeze = contrarian bullish
                if sp < 5:
                    short_score = 15
        ...
        # normalize: max possible = 25+20+25+15+15 = 100, min = -45
        clamped = max(0, min(100, total_score + 45))  # shift so 0 bullish => 45, neutral ~55
...
@mcp.tool()
def get_options_flow_heatmap() -> dict:
    """
    ...
    # Representative tickers per sector (3-4 per sector for robust signal)
    SECTOR_TICKERS = {
    ...
    def fetch_options_data(ticker: str) -> dict:
        try:
            ...
            call_oi = int(calls["openInterest"].fillna(0).sum())
            put_oi = int(puts["openInterest"].fillna(0).sum())
            # Unusual sweep detection: high volume/OI ratio
    ...
    try:
        ...
        # Sort sectors by pc_ratio (ascending = most bullish first)
...
@mcp.tool()
def get_market_regime_composite() -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            if not vix_hist.empty:
                ...
                vix_ma20 = float(vix_hist["Close"].tail(20).mean())
                if vix_level < 15:
                    vix_score = 25
        ...
        # ── Layer 2: Yield Curve Shape (10Y - 2Y proxy) ──
        curve_score = 0
        curve_shape = "UNKNOWN"
        spread_bps = None
        try:
            tnx = yf.Ticker("^TNX")  # 10Y
            irx = yf.Ticker("^IRX")  # 3M (proxy for short end)
            ...
            if not tnx_hist.empty and not irx_hist.empty:
                ...
                spread = y10 - y3m  # bps-equivalent (already in percent)
        ...
        # ── Layer 3: Credit Spreads (HYG vs IEF proxy) ──
        credit_score = 0
        ...
        # ── Layer 4: Market Breadth (S&P 500 proxy via SPY/RSP) ──
        breadth_score = 0
        ...
        try:
            ...
            if not spy_hist.empty and not rsp_hist.empty and len(spy_hist) >= 20:
                # Compare 20d performance: RSP outperforming = broad participation (healthy breadth)
        ...
        # ── Layer 5: Sector Rotation (defensive vs cyclical) ──
        rotation_score = 0
        rotation_signal = "UNKNOWN"
        try:
            # Cyclical: XLK (Tech) + XLY (Cons Disc)
            # Defensive: XLP (Cons Staples) + XLU (Utilities)
        ...
        max_possible = 25 + 20 + 20 + 20 + 15  # = 100
        min_possible = -25 - 20 - 20 - 15 - 15  # = -95
        # Normalize to 0-100
...
@mcp.tool()
def get_earnings_surprise_vs_sector(ticker: str) -> dict:
    ...
    try:
        ...
        # Sector peer universes (representative S&P 500 stocks per sector)
        SECTOR_PEERS = {
        ...
        def _get_eps_surprises(sym):
            """Fetch last 4 quarters of EPS surprise % for a symbol."""
            try:
                t = yf.Ticker(sym)
                ...
                for _, row in hist.iterrows():
                    try:
                        ...
                        continue
                return surprises[:4]  # last 4 quarters
            except Exception:
                return []

        # Get ticker info (sector)
        t_obj = yf.Ticker(ticker)
        info = t_obj.info or {}
        sector = info.get("sector", "Unknown")

        # Get ticker's own EPS surprises
        ...
        if not peers:
            # Try to use a broader set
            all_peers = []
            for v in SECTOR_PEERS.values():
                all_peers.extend(v)
            peers = [p for p in all_peers if p != ticker][:20]

        # Fetch peer surprises in parallel
        peer_surprises_raw = []
        with ThreadPoolExecutor(max_workers=8) as ex:
            results = list(ex.map(_get_eps_surprises, peers[:15]))
        for r in results:
            if r:
        ...
        if peer_surprises_raw:
            ...
            sector_avg_surprise = 4.5  # Historical S&P 500 average
...
@mcp.tool()
def get_52w_high_low_momentum() -> dict:
    ...
    try:
        ...
        # S&P 500 proxy universe by sector (150 stocks total)
        UNIVERSE = {
        ...
        def _check_52w(sym):
            try:
                ...
                # Within 3% of 52w high = "new high zone"
                # Within 3% of 52w low = "new low zone"
        ...
        with ThreadPoolExecutor(max_workers=12) as ex:
            results = list(ex.map(_check_52w, all_tickers))
...
@mcp.tool()
def get_options_iv_percentile(ticker: str = "AAPL") -> dict:
...
@mcp.tool()
def get_earnings_date_countdown(ticker: str = "AAPL") -> dict:
    ...
    try:
        ...
        # ── Pre-earnings price move pattern (5d before vs. 1d after) ──
        ...
        if days_until is not None:
            if days_until <= 0:
                ...
                timing_notes.append("10-21 days out. IV beginning to rise. Long options favorable vs. short premium (too early for condors).")
            elif days_until <= 45:
                timing_signal = "WATCH_LIST"
                timing_notes.append("21-45 days out. Monitor IV rank; consider entering on IV dips or pre-catalyst setups.")
            else:
                timing_signal = "TOO_EARLY"
                timing_notes.append("More than 45 days out. Wait for IV to begin expanding before positioning.")

        if beat_rate >= 75:
            timing_notes.append(f"Strong beat history ({beat_rate}% beat rate, avg +{avg_surprise}% surprise). Consider bullish bias post-earnings.")
        elif beat_rate <= 40:
        ...
        return {
            ...
            "strategy_guide": {
                "straddle": f"Buy ATM straddle 7-10 days before earnings. Close 1 day before or day-of to avoid IV crush.",
...
@mcp.tool()
def get_sector_etf_vs_spy_beta() -> dict:
    ...
    try:
        ...
        for sym, name in SECTORS.items():
            ...
            try:
                ...
                # 1W/1M/3M returns for momentum context
                h = yf.Ticker(sym).history(period="1y", interval="1d")
                closes = h["Close"].dropna()
                cur = float(closes.iloc[-1])
                ret_1w = round((cur / float(closes.iloc[-6]) - 1) * 100, 2) if len(closes) >= 6 else None
                ret_1m = round((cur / float(closes.iloc[-22]) - 1) * 100, 2) if len(closes) >= 22 else None
                ret_3m = round((cur / float(closes.iloc[-66]) - 1) * 100, 2) if len(closes) >= 66 else None
        ...
        # Market regime: compare SPY 1M and 3M trend
...
@mcp.tool()
def get_relative_strength_ranking(tickers: str = "AAPL,MSFT,NVDA,GOOGL,AMZN,META,TSLA,JPM,V,UNH") -> dict:
    ...
    try:
        ...
        for sym in ticker_list:
            ...
            # RS score = excess return vs SPY (each period)
            ...
            # Composite momentum score (weighted avg of returns)
            comp_parts = []
        ...
        # Rank by composite momentum descending
        ...
        # Market context: what's SPY doing?
        if spy_1m > 2:
...
@mcp.tool()
def get_fear_greed_composite() -> dict:
    ...
    try:
        ...
        def fetch_hist(sym):
            try:
                period = "1y" if sym not in ["^VIX"] else "1y"
        ...
        # ── Component 1: VIX Regime (20 pts) ──────────────────────────────
        vix_closes = results.get("^VIX")
        vix_score = 10  # neutral default
        vix_val = None
        vix_label = "UNKNOWN"
        if vix_closes is not None and len(vix_closes) >= 10:
            vix_val = round(float(vix_closes.iloc[-1]), 2)
            # Low VIX = greed, high VIX = fear
            # VIX < 13: extreme greed (20), 13-18: greed (15), 18-24: neutral (10), 24-32: fear (5), >32: extreme fear (0)
            if vix_val < 13:
        ...
        # ── Component 2: 52W High/Low Breadth (20 pts) ────────────────────
        ...
        breadth_score = 10  # neutral default
        ...
        # ── Component 3: Safe Haven Demand (15 pts) — inverted ────────────
        tlt_closes = results.get("TLT")
        spy_closes = results.get("SPY")
        safe_score = 7  # neutral
        safe_label = "NEUTRAL"
        tlt_vs_spy_30d = None
        if tlt_closes is not None and spy_closes is not None and len(tlt_closes) >= 22 and len(spy_closes) >= 22:
            ...
            # High TLT vs SPY = fear (bonds outperform), inverted for greed score
            if tlt_vs_spy_30d <= -5:
        ...
        # ── Component 4: Put/Call Ratio (15 pts) — inverted ───────────────
        pc_score = 7  # neutral
        pc_label = "NEUTRAL"
        pc_ratio = None
        try:
            ...
            if exps:
                ...
                if best_exp:
                    ...
                    if call_vol > 0:
                        ...
                        # High P/C = fear, inverted for greed score
                        if pc_ratio <= 0.6:
        ...
        # ── Component 5: Junk Bond Demand (15 pts) ────────────────────────
        hyg_closes = results.get("HYG")
        ief_closes = results.get("IEF")
        junk_score = 7  # neutral
        junk_label = "NEUTRAL"
        hyg_vs_ief_30d = None
        if hyg_closes is not None and ief_closes is not None and len(hyg_closes) >= 22 and len(ief_closes) >= 22:
            ...
            # HYG outperforming IEF = risk appetite = greed
            if hyg_vs_ief_30d >= 3:
        ...
        # ── Component 6: Market Momentum (10 pts) ─────────────────────────
        ...
        # ── Component 7: Stock Price Breadth (5 pts) ──────────────────────
        rsp_closes = results.get("RSP")
        breadth2_score = 2  # neutral
        ...
        if spy_closes is not None and rsp_closes is not None and len(spy_closes) >= 22 and len(rsp_closes) >= 22:
            ...
            # RSP (equal-weight) outperforming SPY = broad participation = greed
            if spy_vs_rsp_30d >= 2:
        ...
        max_possible = 20 + 20 + 15 + 15 + 15 + 10 + 5  # = 100
...
@mcp.tool()
def get_momentum_factor_screen(top_n: int = 20) -> dict:
    """
    ...
    # 250-stock S&P 500 proxy universe with sector labels
    UNIVERSE = {
        # Technology (XLK)
    ...
    def fetch_momentum(ticker):
        try:
            ...
            # 6m return for secondary ranking
...
@mcp.tool()
def get_economic_indicators_dashboard(country: str = "US") -> dict:
    """
    Macro economic health dashboard using public ETF proxies and World Bank open data.
    Scores GDP growth, inflation, employment, trade balance, and fiscal health.
    No API key required.
    ...
    # Country ETF proxy map: equity / bond / currency
    COUNTRY_CONFIG = {
        "US":  {"name": "United States",  "equity": "SPY",  "bond": "TLT",   "fx": "UUP",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "USA"},
        "EU":  {"name": "Euro Area",      "equity": "EZU",  "bond": "BNDX",  "fx": "FXE",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "EUU"},
        "JP":  {"name": "Japan",          "equity": "EWJ",  "bond": "BNDX",  "fx": "FXY",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "JPN"},
        "CN":  {"name": "China",          "equity": "MCHI", "bond": "BNDX",  "fx": "CYB",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "CHN"},
        "KR":  {"name": "South Korea",    "equity": "EWY",  "bond": "BNDX",  "fx": "FXI",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "KOR"},
        "GB":  {"name": "United Kingdom", "equity": "EWU",  "bond": "BNDX",  "fx": "FXB",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "GBR"},
        "CA":  {"name": "Canada",         "equity": "EWC",  "bond": "BNDX",  "fx": "FXC",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "CAN"},
        "AU":  {"name": "Australia",      "equity": "EWA",  "bond": "BNDX",  "fx": "FXA",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "AUS"},
        "IN":  {"name": "India",          "equity": "INDA", "bond": "BNDX",  "fx": "ICN",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "IND"},
        "BR":  {"name": "Brazil",         "equity": "EWZ",  "bond": "BNDX",  "fx": "BZF",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "BRA"},
        "MX":  {"name": "Mexico",         "equity": "EWW",  "bond": "BNDX",  "fx": "FXM",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "MEX"},
        "DE":  {"name": "Germany",        "equity": "EWG",  "bond": "BNDX",  "fx": "FXE",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "DEU"},
        "FR":  {"name": "France",         "equity": "EWQ",  "bond": "BNDX",  "fx": "FXE",   "gdp_code": "NY.GDP.MKTP.KD.ZG", "wb_code": "FRA"},
    }
    ...
    # ── Component 1: Growth Proxy (25 pts) — equity ETF 1Y + 3M momentum ──
    ...
    # ── Component 2: Inflation Proxy (20 pts) — GLD + USO composite ──
    ...
    try:
        ...
        # Gold + Oil composite = inflation proxy
        infl_proxy = ((gold_ret_3m or 0) * 0.5 + (oil_ret_3m or 0) * 0.5)
        # Moderate inflation (2-4%) = good. Too high or deflation = bad.
        # We score: moderate positive = best, high = bad, negative = deflation risk
        if -5 <= infl_proxy <= 10:
    ...
    # ── Component 3: Employment / Activity Proxy (20 pts) — equity breadth + consumer ETFs ──
    ...
    try:
        ...
        # Discretionary outperforming staples = healthy employment / consumer
    ...
    # ── Component 4: Fiscal / Credit Health (20 pts) — HYG vs IEF + VIX ──
    ...
    try:
        ...
        # Combine: credit spread proxy + VIX
    ...
    # ── Component 5: External Balance / Currency (15 pts) ──
...
@mcp.tool()
def get_earnings_quality_score(ticker: str) -> dict:
        ...
        if cf is not None and inc is not None and not cf.empty and not inc.empty:
            ...
            if ocf_row is not None and ni_row is not None:
                ...
                if len(ocf_vals) >= 1 and len(ni_vals) >= 1:
                    ...
                    # Accruals ratio = (NI - OCF) / avg assets
                    ...
                    if avg_assets and avg_assets > 0:
                        ...
                        # Low accruals = high quality (NI backed by cash)
                        if accruals_ratio <= -5:
    ...
    try:
        ...
        if inc is not None and not inc.empty:
            ...
            if ni_row is not None:
                ni_vals = ni_row.dropna().values[:4]  # last 4 quarters/years
                if len(ni_vals) >= 3:
        ...
        if inc is not None and not inc.empty:
            ...
            if rev_row is not None:
                ...
                if len(rv) >= 2:
                    ...
                    # YoY growth (most recent vs prior)
                    if rv[1] > 0:
    ...
    try:
        info = tk.info
        roe_latest = info.get("returnOnEquity", None)
...
@mcp.tool()
def get_market_internals_dashboard() -> dict:
    ...
    def fetch_ticker_internals(sym):
        try:
            ...
            # McClellan proxy: 19d EMA vs 39d EMA of daily advances
            # Use price momentum as proxy (10d vs 30d returns)
    ...
    # Use 10d vs 30d breadth ratio as proxy
    ...
    # ── Bullish Percent Index Proxy ───────────────────────────────────────
    # BPI proxy: % above 200d MA (long-term bullish)
...
@mcp.tool()
def get_dividend_safety_screen(sector: str = "ALL") -> dict:
    ...
    def analyze_stock(sym, sec):
        try:
            ...
            five_yr_div_growth = info.get("fiveYearAvgDividendYield") or None
            forward_pe = info.get("forwardPE") or None
            beta = info.get("beta") or 1.0

            # Payout ratio score (0-30 pts)
            pr_score = 0
            pr_label = "UNKNOWN"
            if payout_ratio is not None and payout_ratio > 0:
                if payout_ratio <= 0.40:
                    pr_score = 30
                    pr_label = "VERY_LOW_PAYOUT"
                elif payout_ratio <= 0.60:
                    pr_score = 25
                    pr_label = "SUSTAINABLE_PAYOUT"
                elif payout_ratio <= 0.75:
                    pr_score = 15
                    pr_label = "ELEVATED_PAYOUT"
                elif payout_ratio <= 0.90:
                    pr_score = 5
                    pr_label = "HIGH_PAYOUT"
                elif payout_ratio <= 1.10:
                    pr_score = 0
                    pr_label = "DANGER_ZONE"
                else:
                    pr_score = -10
                    pr_label = "UNSUSTAINABLE_PAYOUT"
            elif sec in ("Utilities", "RealEstate"):
                pr_score = 15
                pr_label = "SECTOR_TYPICAL_HIGH"

            # FCF coverage score (0-35 pts)
            fcf_score = 0
            fcf_label = "UNKNOWN"
            fcf_coverage = None
            if fcf_per_share and shares and div_rate > 0:
                fcf_per_share_val = fcf_per_share / shares if shares else 0
                fcf_coverage = fcf_per_share_val / div_rate if div_rate > 0 else None
                if fcf_coverage is not None:
                    if fcf_coverage >= 3.0:
                        fcf_score = 35
                        fcf_label = "EXCELLENT_FCF_COVERAGE"
                    elif fcf_coverage >= 2.0:
                        fcf_score = 28
                        fcf_label = "STRONG_FCF_COVERAGE"
                    elif fcf_coverage >= 1.3:
                        fcf_score = 20
                        fcf_label = "ADEQUATE_FCF_COVERAGE"
                    elif fcf_coverage >= 1.0:
                        fcf_score = 10
                        fcf_label = "TIGHT_FCF_COVERAGE"
                    elif fcf_coverage >= 0.7:
                        fcf_score = 0
                        fcf_label = "INSUFFICIENT_FCF"
                    else:
                        fcf_score = -15
                        fcf_label = "FCF_DEFICIT_RISK"
            else:
                if eps_ttm > 0 and div_rate > 0:
                    earnings_coverage = eps_ttm / div_rate
                    if earnings_coverage >= 2.0:
                        fcf_score = 25
                        fcf_label = "EARNINGS_COVERED"
                    elif earnings_coverage >= 1.2:
                        fcf_score = 15
                        fcf_label = "EARNINGS_MARGINAL"
                    else:
                        fcf_score = 0
                        fcf_label = "EARNINGS_TIGHT"

            # Dividend yield quality (0-20 pts)
            yield_score = 0
            yield_label = "NO_DIVIDEND"
            if div_yield > 0:
                if 0.015 <= div_yield <= 0.04:
                    yield_score = 20
                    yield_label = "ATTRACTIVE_YIELD"
                elif div_yield < 0.015:
                    yield_score = 12
                    yield_label = "LOW_YIELD"
                elif div_yield <= 0.06:
                    yield_score = 15
                    yield_label = "HIGH_YIELD_WATCH"
                else:
                    yield_score = 5
                    yield_label = "YIELD_TRAP_RISK"

            # Stability proxy: beta-adjusted (0-15 pts)
            stability_score = 0
            if beta is not None:
                if beta <= 0.6:
                    stability_score = 15
            ...
            total_score = pr_score + fcf_score + yield_score + stability_score
            total_score = max(0, min(100, total_score))

            if total_score >= 65:
                safety_signal = "SAFE_DIVIDEND"
                cut_risk = "LOW"
            elif total_score >= 40:
                safety_signal = "AT_RISK"
                cut_risk = "MODERATE"
            else:
                safety_signal = "DIVIDEND_CUT_RISK"
                cut_risk = "HIGH"
            ...
            return {
                ...
                "current_price": round(current_price, 2),
                "score_breakdown": {
                    "payout_ratio_pts": pr_score,
                    "fcf_coverage_pts": fcf_score,
                    "yield_quality_pts": yield_score,
                    "stability_pts": stability_score,
                    "max_pts": 100,
                },
            }
        except Exception:
    ...
    return {
        ...
        "filter_sector": sector,
        "summary": {
            "safe_dividend": len(safe),
            "at_risk": len(at_risk),
            "dividend_cut_risk": len(cut_risk_list),
        },
...
@mcp.tool()
def get_valuation_composite(ticker: str) -> dict:
    ...
    if pe_used and pe_used > 0 and sector_pe_avg:
        ...
        if pe_ratio <= 0.6:
            pe_score = 25
            pe_label = "DEEPLY_CHEAP_VS_SECTOR"
        elif pe_ratio <= 0.85:
            pe_score = 20
            pe_label = "CHEAP_VS_SECTOR"
        elif pe_ratio <= 1.15:
            pe_score = 12
            pe_label = "IN_LINE_WITH_SECTOR"
        elif pe_ratio <= 1.5:
            pe_score = 5
            pe_label = "EXPENSIVE_VS_SECTOR"
        else:
            pe_score = 0
            pe_label = "SIGNIFICANTLY_EXPENSIVE"
    ...
    if pb and pb > 0 and sector_pb_avg:
        ...
        if pb_ratio <= 0.6:
            ...
            pb_score = 5
            pb_label = "ELEVATED_PB"
        else:
            pb_score = 0
            pb_label = "EXPENSIVE_PB"

    # ── EV/EBITDA score (0-20 pts) ───────────────────────────────────────
    ev_score = 0
    ev_label = "NO_EV_DATA"
    if ev_ebitda and ev_ebitda > 0 and sector_ev_ebitda_avg:
        ...
        if ev_ratio <= 0.6:
            ...
            ev_score = 4
            ev_label = "ELEVATED_EV"
        else:
            ev_score = 0
            ev_label = "EXPENSIVE_EV"

    # ── PEG score (0-15 pts) ─────────────────────────────────────────────
    peg_score = 0
    peg_label = "NO_PEG_DATA"
    if peg and peg > 0:
        if peg <= 0.8:
            peg_score = 15
            peg_label = "GROWTH_AT_DISCOUNT"
        elif peg <= 1.2:
            peg_score = 11
            peg_label = "FAIR_GROWTH_PRICE"
        elif peg <= 2.0:
            peg_score = 5
            peg_label = "EXPENSIVE_GROWTH"
        else:
            peg_score = 0
            peg_label = "GROWTH_OVERPRICED"

    # ── Simple DCF proxy score (0-20 pts) ────────────────────────────────
    dcf_score = 0
    dcf_label = "NO_DCF_DATA"
    dcf_implied = None
    dcf_upside_pct = None
    ...
    if fcf and shares and shares > 0 and current_price > 0:
        ...
        # 10-year DCF simplified: sum of growing FCF / (1+r)^t + terminal value
        try:
            pv_fcf = 0.0
            cf = fcf_per_share
            for t in range(1, 11):
            ...
            if dcf_upside_pct >= 40:
                dcf_score = 20
                dcf_label = "DEEPLY_UNDERVALUED_DCF"
            elif dcf_upside_pct >= 20:
                dcf_score = 16
                dcf_label = "UNDERVALUED_DCF"
            elif dcf_upside_pct >= -10:
                dcf_score = 10
                dcf_label = "FAIRLY_VALUED_DCF"
            elif dcf_upside_pct >= -25:
                dcf_score = 4
                dcf_label = "OVERVALUED_DCF"
            else:
                dcf_score = 0
                dcf_label = "SIGNIFICANTLY_OVERVALUED_DCF"
        except Exception:
            pass

    total_score = pe_score + pb_score + ev_score + peg_score + dcf_score
    total_score = max(0, min(100, total_score))
...
@mcp.tool()
def get_earnings_growth_tracker(ticker: str) -> dict:
    ...
    # EPS history from earnings_history
    eps_history = []
    try:
...
@mcp.tool()
def get_liquidity_score(ticker: str) -> dict:
    ...
    - Bid-Ask Spread Proxy (15pts): estimated from 30d realized volatility
...
@mcp.tool()
def get_quality_factor_screen(top_n: int = 20) -> dict:
    """
    ...
    def fetch_quality(ticker):
        try:
            ...
            # EPS CAGR proxy: use earnings history
            try:
                hist = t.earnings_history
            ...
            # 3yr EPS CAGR from quarterly → annualize
            eps_cagr = None
            if len(eps_vals) >= 4:
                ...
                if old_annual and old_annual > 0 and recent_annual > 0:
                    years = 2.0
                    eps_cagr = (recent_annual / old_annual) ** (1 / years) - 1
            ...
            # Earnings stability proxy: profit margin consistency
            try:
                fin = t.quarterly_financials
                if fin is not None and not fin.empty and "Net Income" in fin.index:
                    ni_vals = fin.loc["Net Income"].dropna().tolist()[:8]
                    if len(ni_vals) >= 4:
                        positives = sum(1 for x in ni_vals if x > 0)
                        stability_rate = positives / len(ni_vals)
                    else:
                        stability_rate = None
                else:
                    stability_rate = None
            ...
            # Score ROE (25pts)
            if roe is not None:
                if roe >= 0.20: roe_score = 25
                elif roe >= 0.15: roe_score = 20
                elif roe >= 0.10: roe_score = 14
                elif roe >= 0.00: roe_score = 6
                else: roe_score = 0
            else:
                roe_score = 8  # neutral if missing

            # Score D/E (25pts)
            if de is not None:
                de_normalized = de / 100 if de > 10 else de
                if de_normalized <= 0.3: de_score = 25
                elif de_normalized <= 0.6: de_score = 20
                elif de_normalized <= 1.0: de_score = 14
                elif de_normalized <= 2.0: de_score = 6
                else: de_score = 0
            else:
                de_score = 10  # neutral

            # Score EPS CAGR (20pts)
            if eps_cagr is not None:
                if eps_cagr >= 0.20: eps_score = 20
                elif eps_cagr >= 0.12: eps_score = 16
                elif eps_cagr >= 0.06: eps_score = 11
                elif eps_cagr >= 0.00: eps_score = 5
                else: eps_score = 0
            else:
                eps_score = 8  # neutral

            # Score FCF margin (20pts)
            if fcf_margin is not None:
                if fcf_margin >= 0.20: fcf_score = 20
                elif fcf_margin >= 0.12: fcf_score = 16
                elif fcf_margin >= 0.06: fcf_score = 11
                elif fcf_margin >= 0.00: fcf_score = 5
                else: fcf_score = 0
            else:
                fcf_score = 8  # neutral

            # Score earnings stability (10pts)
            if stability_rate is not None:
                if stability_rate >= 0.90: stab_score = 10
                elif stability_rate >= 0.75: stab_score = 8
                elif stability_rate >= 0.60: stab_score = 5
                elif stability_rate >= 0.40: stab_score = 2
                else: stab_score = 0
            else:
                stab_score = 5  # neutral

            total = roe_score + de_score + eps_score + fcf_score + stab_score

            if total >= 85: quality_signal = "QUALITY_ELITE"
            elif total >= 65: quality_signal = "QUALITY_SOLID"
            elif total >= 40: quality_signal = "QUALITY_MIXED"
            else: quality_signal = "LOW_QUALITY"
            ...
            return {
                ...
                "components": {
                    ...
                    "earnings_stability_score": stab_score,
                },
                "raw_metrics": {
    ...
    quality_solid = [r for r in results if r["quality_signal"] == "QUALITY_SOLID"]
    low_quality = [r for r in results if r["quality_signal"] == "LOW_QUALITY"]

    # Sector breakdown of top-N
    sector_counts = {}
    for r in top:
    ...
    return {
        ...
        "scoring_guide": {
            ...
            "earnings_stability": "10pts — Quarterly NI positive rate ≥90%=10 / ≥75%=8 / ≥60%=5 / ≥40%=2 / <40%=0",
        },
        "strategy_guide": "QUALITY_ELITE: Strong moat + capital efficiency. Ideal for long-term core holdings. QUALITY_SOLID: Good fundamentals, some trade-offs. QUALITY_MIXED: Cyclical or transitional businesses. LOW_QUALITY: Avoid or short candidates.",
        "source": "Yahoo Finance public data — no API key required",
        "note": "Combine with get_valuation_composite() for GARP (Growth at Reasonable Price) strategy."
    }


@mcp.tool()
def get_capital_allocation_score(ticker: str) -> dict:
    """
    ...
    try:
        ...
        # --- Buyback Yield (25pts) ---
        # Use cashflowStatement for repurchaseOfStock
        try:
            cf = t.cashflow  # annual
            if cf is not None and not cf.empty:
                repurchase = None
                for key in ["Repurchase Of Capital Stock", "Common Stock Repurchased", "Purchase Of Business"]:
                    if key in cf.index:
                        vals = cf.loc[key].dropna().tolist()
                        if vals:
                            repurchase = vals[0]  # most recent year
                            break
                # Also check shares trend
                shares_trend = None
                for key in ["Common Stock Shares Outstanding"]:
                    if key in cf.index:
                        pass
            else:
                repurchase = None
        ...
        # Use shares outstanding from balance sheet for trend
        try:
            bs = t.balance_sheet
            if bs is not None and not bs.empty:
                ...
                for key in ["Ordinary Shares Number", "Common Stock"]:
                    if key in bs.index:
                        ...
                        if len(vals) >= 2 and vals[0] and vals[1]:
                            # positive = issuing, negative = buybacks
                            shares_change = (vals[0] - vals[1]) / abs(vals[1])
                            ordinary_shares = shares_change
                            break
            else:
                ordinary_shares = None
        except Exception:
        ...
        # Buyback yield via repurchase / market cap
        buyback_yield = None
        ...
        if buyback_yield is not None:
            if buyback_yield >= 0.05: bb_score = 25  # ≥5% yield — aggressive buybacks
            elif buyback_yield >= 0.03: bb_score = 20
            elif buyback_yield >= 0.01: bb_score = 14
            elif buyback_yield >= 0.001: bb_score = 8
            else: bb_score = 3  # minimal buybacks
        elif ordinary_shares is not None:
            if ordinary_shares <= -0.03: bb_score = 22  # shares shrinking ≥3%
            elif ordinary_shares <= -0.01: bb_score = 16
            elif ordinary_shares <= 0.01: bb_score = 10
            else: bb_score = 3  # dilution
        else:
            bb_score = 10  # neutral

        # --- Dividend Growth Consistency (20pts) ---
        ...
        try:
            ...
            if div_history is not None and len(div_history) >= 8:
                # Check YoY dividend growth consistency
        ...
        # Score dividend (20pts)
        if not has_dividends and div_rate == 0:
            # No dividend — check if it's a growth company (reasonable)
            if sector in ["Technology", "Consumer Discretionary", "Communication Services"]:
                div_score = 14  # neutral for growth sectors
            else:
                div_score = 8
        else:
            # Has dividend
            if div_growth >= 0.08 and payout_ratio <= 0.60: div_score = 20  # growing + sustainable
            elif div_growth >= 0.04 and payout_ratio <= 0.70: div_score = 16
            elif div_growth >= 0.00 and payout_ratio <= 0.80: div_score = 11
            elif payout_ratio > 0.90: div_score = 4  # unsustainable
            else: div_score = 8

        # --- FCF Reinvestment Ratio (20pts) ---
        fcf = info.get("freeCashflow")
        ...
        # Score FCF reinvestment (20pts)
        # Optimal: reinvest 30-60% of FCF (balanced growth)
        if fcf_reinvest_ratio is not None:
            if 0.20 <= fcf_reinvest_ratio <= 0.60: fcf_reinvest_score = 20  # balanced
            elif 0.10 <= fcf_reinvest_ratio < 0.20: fcf_reinvest_score = 16  # light but positive
            elif 0.60 < fcf_reinvest_ratio <= 0.90: fcf_reinvest_score = 12  # heavy but manageable
            elif fcf_reinvest_ratio > 0.90: fcf_reinvest_score = 5  # capex eating FCF
            elif fcf_reinvest_ratio < 0.10: fcf_reinvest_score = 12  # very asset-light (also fine)
            else: fcf_reinvest_score = 8
        elif fcf and fcf > 0:
            fcf_reinvest_score = 14  # has FCF but capex unknown
        else:
            fcf_reinvest_score = 5  # no FCF

        # --- M&A Discipline via Goodwill (20pts) ---
        try:
            bs_annual = t.balance_sheet
            goodwill_score_val = 15  # default neutral
            goodwill_ratio = None
            ...
            if goodwill_ratio is not None:
                if goodwill_ratio <= 0.05: goodwill_score_val = 20  # minimal goodwill — organic grower
                elif goodwill_ratio <= 0.15: goodwill_score_val = 16
                elif goodwill_ratio <= 0.30: goodwill_score_val = 11
                elif goodwill_ratio <= 0.50: goodwill_score_val = 6
                else: goodwill_score_val = 2  # goodwill-heavy (acquisition risk)

                # Bonus/penalty for growth rate
                if goodwill_growth is not None:
                    if goodwill_growth > 0.20:  # rapid M&A
                        goodwill_score_val = max(0, goodwill_score_val - 3)
                    elif goodwill_growth < 0:  # amortization/impairment
                        goodwill_score_val = max(0, goodwill_score_val - 2)
        except Exception:
        ...
        # Score ROIC (15pts)
        if roic is not None:
            if roic >= 0.20: roic_score = 15   # ROIC ≥ 20% — exceptional
            elif roic >= 0.15: roic_score = 12
            elif roic >= 0.10: roic_score = 9
            elif roic >= 0.05: roic_score = 5
            else: roic_score = 2
        else:
            roic_score = 7  # neutral

        total_score = bb_score + div_score + fcf_reinvest_score + goodwill_score_val + roic_score

        if total_score >= 80: capital_signal = "EXCELLENT_CAPITAL_ALLOCATOR"
        elif total_score >= 60: capital_signal = "GOOD_ALLOCATOR"
        elif total_score >= 40: capital_signal = "MIXED_ALLOCATOR"
        else: capital_signal = "POOR_ALLOCATOR"

        action_guide = {
            "EXCELLENT_CAPITAL_ALLOCATOR": "Strong shareholder value creation. Management consistently returns capital efficiently. Premium valuation justified.",
            "GOOD_ALLOCATOR": "Above-average capital discipline. Solid buyback/dividend track record with controlled M&A. Good long-term holding.",
            "MIXED_ALLOCATOR": "Inconsistent capital allocation. Monitor for goodwill impairment or excessive dilution. Needs case-by-case analysis.",
            "POOR_ALLOCATOR": "Capital destruction risk. High M&A spending, weak FCF, or dilutive equity issuance. Avoid unless turnaround thesis.",
        }[capital_signal]
        ...
        return {
            ...
            "score_breakdown": {
                "buyback_yield": {
                    "score": bb_score, "max": 25,
                    "buyback_yield_pct": round(buyback_yield * 100, 2) if buyback_yield else None,
                    "shares_outstanding_change_pct": round(ordinary_shares * 100, 2) if ordinary_shares else None,
                    "note": "Negative shares change = buybacks reducing dilution."
                },
                "dividend_growth_consistency": {
                    "score": div_score, "max": 20,
                    "dividend_yield_pct": round(div_yield * 100, 2) if div_yield else None,
                    "dividend_growth_yoy_pct": round(div_growth * 100, 1) if div_growth else None,
                    "payout_ratio_pct": round(payout_ratio * 100, 1) if payout_ratio else None,
                    "has_dividends": has_dividends,
                },
                "fcf_reinvestment_ratio": {
                    "score": fcf_reinvest_score, "max": 20,
                    "capex_to_fcf_ratio": round(fcf_reinvest_ratio, 2) if fcf_reinvest_ratio else None,
                    "fcf_M": round(fcf / 1e6, 1) if fcf else None,
                    "capex_M": round(abs(capex) / 1e6, 1) if capex else None,
                    "note": "Optimal ratio 0.20-0.60 (balanced growth vs. shareholder returns)."
                },
                "ma_discipline_goodwill": {
                    "score": goodwill_score_val, "max": 20,
                    "goodwill_to_assets_pct": round(goodwill_ratio * 100, 1) if goodwill_ratio else None,
                    "goodwill_yoy_growth_pct": round(goodwill_growth * 100, 1) if goodwill_growth is not None else None,
                    "note": "Low goodwill/assets = organic growth or disciplined M&A."
                },
                "roic_proxy": {
                    "score": roic_score, "max": 15,
                    "roic_pct": round(roic * 100, 1) if roic else None,
                    "note": "ROIC = Net Income / Invested Capital proxy. Target ≥10-15% to exceed WACC."
                }
            },
            "signal_thresholds": "EXCELLENT ≥80 | GOOD ≥60 | MIXED ≥40 | POOR <40",
            "source": "Yahoo Finance public data — no API key required",
            "note": "Combine with get_quality_factor_screen() and get_valuation_composite() for full fundamental analysis."
        }
...
@mcp.tool()
def get_management_quality_score(ticker: str) -> dict:
    ...
    try:
        ...
        # --- CEO Proxy (10pts neutral — public data limitation) ---
        ceo_name = "Unknown"
        ...
        for officer in officers:
            ...
            if "chief executive" in title or " ceo" in title:
                ...
                break
        ceo_score = 10  # neutral — tenure not reliably in yfinance
        ...
        # --- Operating Margin Stability (15pts) ---
        op_margin_score = 8
        ...
        return {
            ...
            "score_breakdown": {
                "ceo_tenure_proxy": {
                    "score": ceo_score, "max": 20,
                    "ceo_name": ceo_name,
                    "label": "NEUTRAL_TENURE",
                    "note": "CEO tenure not reliably available via public data. Neutral score applied."
                },
                "roe_trend": {
                    "score": roe_score, "max": 25,
                    "roe_current_pct": round(roe_current * 100, 1) if roe_current else None,
                    "roe_3yr_avg_pct": round(roe_3yr_avg * 100, 1) if roe_3yr_avg else None,
                    "trend": roe_trend,
                    "note": "IMPROVING trend adds +5pts. DECLINING subtracts -3pts."
                },
                "rd_efficiency": {
                    "score": rd_score, "max": 20,
                    "rd_to_revenue_pct": round(rd_to_revenue * 100, 1) if rd_to_revenue else None,
                    "signal": rd_efficiency_signal,
                    "gross_margin_pct": round((info.get("grossMargins") or 0) * 100, 1),
                    "note": "R&D intensity scored relative to gross margin tier."
                },
                "sbc_discipline": {
                    "score": sbc_score, "max": 20,
                    "sbc_to_revenue_pct": round(sbc_to_revenue * 100, 2) if sbc_to_revenue else None,
                    "signal": sbc_signal,
                    "note": "SBC/Revenue ≤2% = low dilution. ≥8% = significant drag on shareholder value."
                },
                "operating_margin_stability": {
                    "score": op_margin_score, "max": 15,
                    "op_margin_3yr_avg_pct": round(op_margin_avg * 100, 1) if op_margin_avg else None,
                    "op_margin_current_pct": round(op_margin_current * 100, 1) if op_margin_current else None,
                    "stability": op_margin_stability,
                    "note": "CV ≤10% and positive average = HIGHLY_STABLE operating margins."
                }
            },
...
@mcp.tool()
def get_competitive_moat_score(ticker: str) -> dict:
    ...
    try:
            ...
            if financials is not None and not financials.empty:
                ...
                if gp_series is not None and rev_series is not None:
                    ...
                    if len(gm_hist) >= 2:
                        ...
                        if len(gm_hist) >= 3:
                            recent_avg = float(np.mean(gm_hist[:2]))
                            older_avg = float(np.mean(gm_hist[2:]))
                            gm_trend = "EXPANDING" if recent_avg > older_avg + 0.02 else ("CONTRACTING" if recent_avg < older_avg - 0.02 else "STABLE")
        ...
        # --- Pricing Power Proxy (20pts) ---
        pp_score = 10
        pp_signal = "UNKNOWN"
        gm_ref = gm_avg if gm_avg else None

        if gm_ref is not None and gm_current:
        ...
        if market_cap >= 200e9:
            mp_score = 15; mp_signal = "DOMINANT_MARKET_POSITION"
        ...
        return {
            ...
            "score_breakdown": {
                "gross_margin_stability": {
                    "score": gm_score, "max": 25,
                    "gross_margin_avg_pct": round(gm_avg * 100, 1) if gm_avg else None,
                    "gross_margin_current_pct": round(gm_current * 100, 1) if gm_current else None,
                    "stability": gm_stability,
                    "trend": gm_trend,
                    "note": "High + stable gross margin = pricing power moat. EXPANDING trend +2pts."
                },
                "operating_leverage": {
                    "score": op_lev_score, "max": 20,
                    "operating_leverage_ratio": round(op_lev_ratio, 2) if op_lev_ratio else None,
                    "signal": op_lev_signal,
                    "note": "Op income growth / Revenue growth. ≥3x = income scales faster than costs."
                },
                "pricing_power_proxy": {
                    "score": pp_score, "max": 20,
                    "current_vs_historical_gm_pp": round((gm_current - gm_ref) * 100, 1) if (gm_current and gm_ref) else None,
                    "signal": pp_signal,
                    "note": "Current gross margin vs 3-5yr historical average. Expanding = pricing power."
                },
                "market_position": {
                    "score": mp_score, "max": 15,
                    "market_cap_B": round(market_cap / 1e9, 1) if market_cap else None,
                    "revenue_growth_yoy_pct": round(rev_growth * 100, 1) if rev_growth else None,
                    "signal": mp_signal,
                    "note": "Market cap tier + revenue growth as scale and position advantage proxy."
                },
                "rd_ip_moat": {
                    "score": rd_moat_score, "max": 20,
                    "signal": rd_moat_signal,
                    "note": "R&D intensity × gross margin quality. High = sustainable innovation barrier to entry."
                }
            },
            "source": "Yahoo Finance public data — no API key required",
...
@mcp.tool()
def get_earnings_revision_momentum(ticker: str) -> dict:
    ...
    try:
        ...
        # Analyst count and recommendation
        ...
        # --- Upgrade/Downgrade history (30/60/90 day windows) ---
        try:
        ...
        # 30d window (most recent, highest weight 50%)
        ...
        revision_score = s30 + s60 + s90  # 0-20 range
        revision_score_100 = round(revision_score * 5)  # normalize to 0-100
        ...
        # --- Macro cycle positioning overlay ---
...
@mcp.tool()
def get_business_cycle_positioning(sector: str = "ALL") -> dict:
    ...
    # --- ETF proxies for cycle detection ---
    cycle_etfs = {
        "cyclical": ["XLY", "XLI", "XLB"],   # Consumer Disc, Industrials, Materials
        "defensive": ["XLP", "XLU", "XLV"],   # Staples, Utilities, Healthcare
        "rate_sensitive": ["TLT", "IEF"],      # Long / Intermediate Treasuries
        "credit": ["HYG", "LQD"],              # High Yield, Investment Grade
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        results = dict(ex.map(fetch_etf, all_etfs))

    def avg_ret(syms, period="ret_3m"):
        vals = [results[s][period] for s in syms if results.get(s)]
        return sum(vals) / len(vals) if vals else 0.0

    # --- Component 1: PMI Proxy (Cyclical vs Defensive rotation) (30pts) ---
    ...
    # --- Component 2: Rate environment (25pts) ---
    tlt = results.get("TLT")
    ief = results.get("IEF")
    if tlt and ief:
        tlt_ret = tlt["ret_3m"]
    ...
    # --- Component 3: Credit spread proxy (25pts) ---
    hyg = results.get("HYG")
    lqd = results.get("LQD")
    if hyg and lqd:
    ...
    # --- Component 4: Earnings / Momentum cycle (20pts) ---
    spy = results.get("SPY")
    qqq = results.get("QQQ")
    if spy and qqq:
    ...
    total_score = pmi_score + rate_env_score + credit_score + momentum_score  # max 100
    ...
    # --- Sector recommendations per cycle phase ---
    sector_guidance = {
        "EARLY_CYCLE": {
    ...
    # Sector-specific signal if requested
    sector_signal = None
    if input_sector != "ALL":
...
def get_peer_comparison(ticker: str) -> dict:
    ...
    # Fetch ticker first to determine sector
    ...
    def percentile_rank(val, peer_vals, higher_is_better=True):
        """Returns percentile of target vs peers (0=worst, 100=best)"""
    ...
    # Top/bottom peers by composite score
    peer_scores = []
    for r in valid:
        pvals = []
        for m in metrics:
...
@mcp.tool()
def get_macro_sensitivity_score(ticker: str) -> dict:
    ...
    NON_FINANCIAL_SECTORS = ["Technology", "Healthcare", "Consumer Discretionary", "Consumer Staples",
    ...
    def fetch_all():
        ...
        return t, info, hist, financials, balance

    macro_etfs = {
        "SPY": None, "TLT": None, "DXY_PROXY": "UUP",
        "XLC": None, "XLY": None,  # Cyclicals
        "XLU": None, "XLP": None,  # Defensives
        "GLD": None, "USO": None,  # Commodities / inflation
    }
    ...
    try:
        t_obj, info, hist, financials, balance = fetch_all()
    except:
        return {"error": f"Could not fetch data for {ticker}"}
    ...
    # 1a. Debt/Equity (leverage = rate sensitivity)
    de_ratio = info.get("debtToEquity")
    if de_ratio is not None:
        de_ratio = de_ratio / 100  # yfinance returns in %, normalize
        if de_ratio <= 0.2:
    ...
    # 1b. TLT correlation (rates falling = TLT rising; negative TLT corr = rate-insensitive)
    ...
    if tlt_data and tlt_data.get("returns") is not None:
        ...
        if len(aligned[0]) >= 60:
            ...
            # High positive TLT corr = rate-sensitive (like bonds/utilities)
            # Negative = equity, not very rate-sensitive via duration
            # We score: less correlated = less rate-sensitive = higher score
            if tlt_corr >= 0.4:
                tlt_pts = 0; tlt_sig = "HIGH_TLT_CORRELATION_DURATION_RISK"
            elif tlt_corr >= 0.2:
                tlt_pts = 0; tlt_sig = "MODERATE_TLT_CORRELATION"
            elif tlt_corr >= -0.1:
                tlt_pts = 0; tlt_sig = "NEUTRAL_RATE_CORRELATION"
            else:
                tlt_pts = 0; tlt_sig = "NEGATIVE_TLT_CORR_EQUITY_LIKE"
        else:
            tlt_corr = None; tlt_sig = "INSUFFICIENT_DATA"; tlt_pts = 0
    else:
        tlt_sig = "TLT_DATA_UNAVAILABLE"; tlt_pts = 0

    rate_score = de_pts  # Primary driver: leverage
    rate_signal_str = de_sig
    rate_detail = {
        "score": rate_score, "max": 25,
        "signal": rate_signal_str,
        "debt_to_equity": round(de_ratio * 100, 1) if de_ratio is not None else None,
        "tlt_correlation": tlt_corr,
        "tlt_signal": tlt_sig,
        "note": "High D/E = high rate sensitivity. TLT correlation provides duration proxy."
    }

    # ── Component 2: Dollar Sensitivity (20 pts) ──
    # Use revenue geography proxy
    ...
    # Strong USD hurts multinational revenue
    if uup_corr is not None:
        if uup_corr <= -0.3:
            usd_pts = 5; usd_sig = "HIGH_USD_SENSITIVITY_MULTINATIONAL"
        elif uup_corr <= -0.1:
            usd_pts = 10; usd_sig = "MODERATE_USD_SENSITIVITY"
        elif uup_corr <= 0.1:
            usd_pts = 15; usd_sig = "LOW_USD_SENSITIVITY_DOMESTIC"
        elif uup_corr <= 0.3:
            usd_pts = 18; usd_sig = "POSITIVE_USD_CORRELATION_USD_BENEFICIARY"
        else:
            usd_pts = 20; usd_sig = "STRONG_USD_BENEFICIARY"
    else:
        usd_pts = 10; usd_sig = "USD_CORRELATION_UNAVAILABLE"

    usd_detail = {
        "score": usd_pts, "max": 20,
        "signal": usd_sig,
        "uup_correlation_1y": uup_corr,
        "note": "Negative UUP correlation = revenue hurt by strong dollar (multinational). Positive = domestic/USD beneficiary."
    }

    # ── Component 3: Inflation Sensitivity (20 pts) ──
    # Proxy: gross margin trend + commodity correlations
    gm = info.get("grossMargins")
    gm_pct = gm * 100 if gm else None

    # Compare gross margin vs commodity performance
    ...
    if gld_data and gld_data.get("returns") is not None:
        ...
        if len(aligned[0]) >= 60:
            gld_corr = round(aligned[0].corr(aligned[1]), 3)
    if uso_data and uso_data.get("returns") is not None:
        aligned = ticker_returns.align(uso_data["returns"], join="inner")
        if len(aligned[0]) >= 60:
            uso_corr = round(aligned[0].corr(aligned[1]), 3)

    # Sectors known to benefit from inflation
    inflation_beneficiary_sectors = ["Energy", "Materials"]
    inflation_hurt_sectors = ["Utilities", "Consumer Discretionary", "Technology"]
    ...
    # Adjust if strong commodity correlation (oil/gold up with ticker = inflation pass-through)
    ...
    infl_detail = {
        "score": infl_pts, "max": 20,
        "signal": infl_sig,
        "gross_margin_pct": round(gm_pct, 1) if gm_pct else None,
        "gold_correlation_1y": gld_corr,
        "oil_correlation_1y": uso_corr,
        "commodity_correlation_avg": commodity_corr_avg,
        "note": "High gross margin = pricing power = inflation hedge. Energy/Materials = direct beneficiaries."
    }

    # ── Component 4: Economic Cycle Sensitivity (35 pts) ──
    # Beta vs SPY + cyclical vs defensive spread
    ...
    if spy_data and spy_data.get("returns") is not None:
        ...
        if len(aligned[0]) >= 120:
            ...
            var = aligned[1].var()
            if var > 0:
                spy_beta = round(cov / var, 3)

    if spy_beta is not None:
        if spy_beta >= 1.5:
            beta_pts = 5; beta_sig = "VERY_HIGH_BETA_EXTREME_CYCLICALITY"
        elif spy_beta >= 1.2:
            beta_pts = 10; beta_sig = "HIGH_BETA_CYCLICAL"
        elif spy_beta >= 0.9:
            beta_pts = 20; beta_sig = "MARKET_BETA_MODERATE_CYCLE"
        elif spy_beta >= 0.6:
            beta_pts = 28; beta_sig = "LOW_BETA_DEFENSIVE"
        else:
            beta_pts = 35; beta_sig = "VERY_LOW_BETA_HIGHLY_DEFENSIVE"
    else:
        beta_pts = 15; beta_sig = "BETA_UNAVAILABLE"

    # Cyclical vs defensive ETF correlation spread
    ...
    if xly_data and xly_data.get("returns") is not None:
        ...
        if len(aligned[0]) >= 60:
            xly_corr = round(aligned[0].corr(aligned[1]), 3)
    if xlu_data and xlu_data.get("returns") is not None:
        aligned = ticker_returns.align(xlu_data["returns"], join="inner")
        if len(aligned[0]) >= 60:
            xlu_corr = round(aligned[0].corr(aligned[1]), 3)

    cyclicality_bias = None
    if xly_corr is not None and xlu_corr is not None:
        cyclicality_bias = round(xly_corr - xlu_corr, 3)
        if cyclicality_bias >= 0.3:
            cycle_bias_sig = "STRONGLY_CYCLICAL_GROWTH_CORRELATED"
        elif cyclicality_bias >= 0.1:
            cycle_bias_sig = "MILDLY_CYCLICAL"
        elif cyclicality_bias >= -0.1:
            cycle_bias_sig = "NEUTRAL_CYCLE_SENSITIVITY"
        elif cyclicality_bias >= -0.3:
            cycle_bias_sig = "MILDLY_DEFENSIVE"
        else:
            cycle_bias_sig = "STRONGLY_DEFENSIVE"
    else:
        cycle_bias_sig = "CYCLE_BIAS_UNAVAILABLE"

    cycle_detail = {
        "score": beta_pts, "max": 35,
        "signal": beta_sig,
        "spy_beta_1y": spy_beta,
        "xly_correlation": xly_corr,
        "xlu_correlation": xlu_corr,
        "cyclicality_bias": cyclicality_bias,
        "cyclicality_signal": cycle_bias_sig,
        "note": "Low beta = defensive = lower economic cycle sensitivity. High XLY corr vs XLU corr = cyclical."
    }

    # ── Composite Score ──
    total_score = rate_score + usd_pts + infl_pts + beta_pts
    total_max = 100

    if total_score >= 75:
        composite_signal = "LOW_MACRO_SENSITIVITY"
        action = "Highly resilient to macro shocks. Suitable for uncertain macro environments. Low rate/dollar/cycle risk."
    elif total_score >= 58:
        composite_signal = "MODERATE_LOW_MACRO_SENSITIVITY"
        action = "Below-average macro sensitivity. Some protection against rate hikes and dollar strength."
    elif total_score >= 42:
        composite_signal = "MODERATE_MACRO_SENSITIVITY"
        action = "Average macro sensitivity. Performance moderately linked to rates, dollar, and economic cycle."
    elif total_score >= 28:
        composite_signal = "HIGH_MACRO_SENSITIVITY"
        action = "Significant macro sensitivity. Rate hikes, strong dollar, or recession will meaningfully impact performance."
    else:
        composite_signal = "EXTREME_MACRO_SENSITIVITY"
        action = "Highly sensitive to macro regime shifts. Avoid in tightening/recessionary environments without hedge."

    # Risk summary
    risk_factors = []
    if rate_score <= 10: risk_factors.append("High leverage → rate-sensitive")
...
@mcp.tool()
def get_earnings_calendar_sector_screen(sector: str = "ALL", days_ahead: int = 14) -> dict:
    """
    ...
    # S&P 500 proxy universe with sector tags
    SECTOR_UNIVERSE = {
    ...
    if sector == "ALL":
        # Mix of 5 stocks per sector for manageable scan
        tickers_to_scan = []
    ...
    def scan_ticker(args):
        ...
        try:
            ...
            if earnings_date is None:
                # Try from info
                next_fye = info.get('nextFiscalYearEnd')
            ...
            # Historical beat rate & avg surprise
            ...
            if beat_rate is not None:
                if beat_rate >= 75:
                    beat_signal = "STRONG_BEAT_HISTORY"
                elif beat_rate >= 55:
                    beat_signal = "MILD_BEAT_HISTORY"
                elif beat_rate >= 40:
                    beat_signal = "NEUTRAL_HISTORY"
                else:
                    beat_signal = "MISS_TENDENCY"
            else:
                beat_signal = "INSUFFICIENT_HISTORY"

            # Forward EPS estimate (consensus)
            forward_eps = info.get('forwardEps')
            trailing_eps = info.get('trailingEps')
            current_price = info.get('currentPrice') or info.get('regularMarketPrice')

            # IV-based expected move proxy (30d realized vol as fallback)
            expected_move_pct = None
            try:
                hist_prices = tk.history(period="3mo")
                if not hist_prices.empty and len(hist_prices) >= 20:
                    log_rets = (hist_prices['Close'] / hist_prices['Close'].shift(1)).apply(lambda x: math.log(x) if x and x > 0 else 0).dropna()
                    rv_30d = log_rets.std() * math.sqrt(252)  # annualized
                    # Expected move for earnings = 1-day RV × sqrt(1) adjusted for event premium (1.5x)
                    expected_move_pct = round(rv_30d / math.sqrt(252) * 100 * 1.5, 1)
            except Exception:
                pass

            # Pre-earnings drift (5-day)
            pre_drift_pct = None
            try:
                hist_1y = tk.history(period="1y")
                if not hist_1y.empty and len(hist_1y) >= 10:
                    # Use current 5-day trend as proxy
                    last_5 = hist_1y['Close'].iloc[-5:]
                    if len(last_5) >= 2:
                        pre_drift_pct = round((last_5.iloc[-1] / last_5.iloc[0] - 1) * 100, 2)
            except Exception:
                pass

            # Timing window
            if days_until == 0:
                timing = "EARNINGS_TODAY"
            elif days_until <= 3:
                timing = "IMMINENT_CATALYST"
            elif days_until <= 7:
                timing = "THIS_WEEK"
            elif days_until <= 14:
                timing = "NEXT_TWO_WEEKS"
            else:
                timing = "UPCOMING"
    ...
    results = []
    with ThreadPoolExecutor(max_workers=12) as ex:
        futures = ex.map(scan_ticker, tickers_to_scan)
        for r in futures:
            if r is not None:
                results.append(r)
    ...
    if not results:
        return {
            ...
            "message": "No upcoming earnings found in the scan window. yfinance calendar data may be limited.",
            "earnings_schedule": [],
            "sector_summary": {},
            "source": "Yahoo Finance — no API key required"
        }
    ...
    return {
        ...
        "source": "Yahoo Finance — no API key required",
        "note": "yfinance earnings calendar accuracy varies. Verify dates via official IR pages for precision trading."
    }


# ── v4.4.0 Tool 2: Alpha Factor Composite ──
@mcp.tool()
def get_alpha_factor_composite(ticker: str) -> dict:
    """
    ...
    try:
        ...
        if not hist.empty and len(hist) >= 130:
            ...
            # 12-1 momentum (12M ago to 1M ago, skip last month to avoid reversal)
            ...
            price_6m = hist['Close'].iloc[-130] if len(hist) >= 130 else hist['Close'].iloc[0]
            ret_3m = (price_now / price_3m - 1) * 100
            ret_6m = (price_now / price_6m - 1) * 100

            # Composite momentum: 12-1 × 50% + 3M × 30% + 6M × 20%
            composite_mom = mom_12_1 * 0.5 + ret_3m * 0.3 + ret_6m * 0.2

            if composite_mom >= 30:
                momentum_pts = 25; mom_sig = "STRONG_MOMENTUM_LEADER"
            elif composite_mom >= 15:
                momentum_pts = 20; mom_sig = "POSITIVE_MOMENTUM"
            elif composite_mom >= 5:
                momentum_pts = 15; mom_sig = "MILD_MOMENTUM"
            elif composite_mom >= -5:
                momentum_pts = 10; mom_sig = "NEUTRAL_MOMENTUM"
            elif composite_mom >= -15:
                momentum_pts = 5; mom_sig = "NEGATIVE_MOMENTUM"
            else:
                momentum_pts = 2; mom_sig = "STRONG_MOMENTUM_LAGGARD"

            # Trend quality: price above 50d and 200d MAs
            ...
            # Trend bonus: +2 if above both MAs
            if above_ma50 and above_ma200:
                momentum_pts = min(25, momentum_pts + 2)
                trend_label = "ABOVE_BOTH_MAS_UPTREND"
            elif above_ma50:
                trend_label = "SHORT_TERM_UPTREND"
            elif above_ma200:
                trend_label = "LONG_TERM_TREND_INTACT"
            else:
                trend_label = "BELOW_BOTH_MAS_DOWNTREND"

            momentum_detail = {
                "score": momentum_pts, "max": 25, "signal": mom_sig,
                "momentum_12_1_pct": round(mom_12_1, 1),
                "return_3m_pct": round(ret_3m, 1),
                "return_6m_pct": round(ret_6m, 1),
                "composite_momentum_pct": round(composite_mom, 1),
                "trend_signal": trend_label,
                "above_ma50": above_ma50,
                "above_ma200": above_ma200,
            }
        else:
            momentum_pts = 10; mom_sig = "INSUFFICIENT_HISTORY"
            momentum_detail = {"score": 10, "max": 25, "signal": mom_sig}
    except Exception as e:
        momentum_pts = 10
        momentum_detail = {"score": 10, "max": 25, "signal": "MOMENTUM_ERROR", "error": str(e)[:80]}
    ...
    try:
        ...
        # ROE sub-score (10 pts)
        roe_pts = 0
        if roe is not None:
            roe_pct = roe * 100
            if roe_pct >= 20: roe_pts = 10; roe_sig = "EXCELLENT_ROE"
            elif roe_pct >= 15: roe_pts = 8; roe_sig = "STRONG_ROE"
            elif roe_pct >= 10: roe_pts = 6; roe_sig = "MODERATE_ROE"
            elif roe_pct >= 0: roe_pts = 3; roe_sig = "LOW_ROE"
            else: roe_pts = 0; roe_sig = "NEGATIVE_ROE"
        else:
            roe_pts = 4; roe_sig = "ROE_UNAVAILABLE"

        # Profit margin sub-score (8 pts)
        margin_pts = 0
        if profit_margin is not None:
            pm_pct = profit_margin * 100
            if pm_pct >= 20: margin_pts = 8; margin_sig = "HIGH_PROFIT_MARGIN"
            elif pm_pct >= 10: margin_pts = 6; margin_sig = "HEALTHY_MARGIN"
            elif pm_pct >= 5: margin_pts = 4; margin_sig = "MODERATE_MARGIN"
            elif pm_pct >= 0: margin_pts = 2; margin_sig = "THIN_MARGIN"
            else: margin_pts = 0; margin_sig = "LOSS_MAKING"
        else:
            margin_pts = 3; margin_sig = "MARGIN_UNAVAILABLE"

        # Balance sheet strength (7 pts): D/E + current ratio
        bs_pts = 0
        if de_ratio is not None:
            if de_ratio <= 30: bs_pts += 4  # Very low leverage
        ...
        quality_pts = 10
        quality_detail = {"score": 10, "max": 25, "signal": "QUALITY_ERROR", "error": str(e)[:80]}
    ...
    try:
        ...
        SECTOR_PE = {
            'Technology': 28, 'Healthcare': 22, 'Financials': 14, 'ConsumerDiscretionary': 24,
            'ConsumerStaples': 20, 'Industrials': 20, 'Energy': 12, 'Materials': 16,
            'Utilities': 18, 'Real Estate': 35, 'Communication Services': 22
        }
        ...
        if pe is not None and pe > 0:
            ...
            if pe_ratio <= 0.6: pe_pts = 10; pe_sig = "DEEPLY_CHEAP"
            elif pe_ratio <= 0.85: pe_pts = 8; pe_sig = "CHEAP"
            elif pe_ratio <= 1.15: pe_pts = 6; pe_sig = "FAIR_VALUE"
            elif pe_ratio <= 1.5: pe_pts = 3; pe_sig = "EXPENSIVE"
            else: pe_pts = 1; pe_sig = "VERY_EXPENSIVE"
        else:
            pe_pts = 4; pe_sig = "PE_UNAVAILABLE"
        ...
        if pb is not None and pb > 0:
            ...
            pb_pts = 3; pb_sig = "PB_UNAVAILABLE"

        # EV/EBITDA score (8 pts)
        ev_pts = 0
        if ev_ebitda is not None and ev_ebitda > 0:
            ...
            ev_pts = 3; ev_sig = "EV_UNAVAILABLE"
        ...
        value_pts = 10
        value_detail = {"score": 10, "max": 25, "signal": "VALUE_ERROR", "error": str(e)[:80]}
    ...
    try:
        ...
        # Revenue growth score (12 pts)
        rev_pts = 0
        if revenue_growth is not None:
            rg_pct = revenue_growth * 100
            if rg_pct >= 25: rev_pts = 12; rev_sig = "HYPERGROWTH_REVENUE"
            elif rg_pct >= 15: rev_pts = 10; rev_sig = "HIGH_GROWTH_REVENUE"
            elif rg_pct >= 8: rev_pts = 8; rev_sig = "SOLID_GROWTH_REVENUE"
            elif rg_pct >= 3: rev_pts = 5; rev_sig = "MODERATE_GROWTH_REVENUE"
            elif rg_pct >= 0: rev_pts = 2; rev_sig = "FLAT_REVENUE"
            else: rev_pts = 0; rev_sig = "DECLINING_REVENUE"
        else:
            rev_pts = 4; rev_sig = "REVENUE_GROWTH_UNAVAILABLE"

        # Earnings growth score (13 pts)
        earn_pts = 0
        if earnings_growth is not None:
            eg_pct = earnings_growth * 100
            if eg_pct >= 30: earn_pts = 13; earn_sig = "HYPERGROWTH_EARNINGS"
            elif eg_pct >= 20: earn_pts = 11; earn_sig = "STRONG_EARNINGS_GROWTH"
            elif eg_pct >= 10: earn_pts = 8; earn_sig = "HEALTHY_EARNINGS_GROWTH"
            elif eg_pct >= 0: earn_pts = 5; earn_sig = "FLAT_EARNINGS"
            ...
            fwd_growth = (eps_fwd / eps_trail - 1) * 100
            if fwd_growth >= 20: earn_pts = 11; earn_sig = "STRONG_FORWARD_EPS_GROWTH"
            elif fwd_growth >= 10: earn_pts = 8; earn_sig = "HEALTHY_FORWARD_EPS_GROWTH"
            elif fwd_growth >= 0: earn_pts = 5; earn_sig = "FLAT_FORWARD_EPS"
            else: earn_pts = 1; earn_sig = "FORWARD_EPS_DECLINE"
        else:
            earn_pts = 5; earn_sig = "EARNINGS_GROWTH_UNAVAILABLE"
        ...
        growth_pts = 10
        growth_detail = {"score": 10, "max": 25, "signal": "GROWTH_ERROR", "error": str(e)[:80]}

    # ── Composite Score ──
    total_score = momentum_pts + quality_pts + value_pts + growth_pts

    if total_score >= 85:
        composite_signal = "EXCEPTIONAL_ALPHA_CANDIDATE"
        action = "Strong buy signal across all factors. Momentum + quality + value + growth aligned. High conviction entry."
    elif total_score >= 70:
        composite_signal = "STRONG_ALPHA_CANDIDATE"
        action = "Most factors favorable. Good risk/reward. Consider position entry."
    elif total_score >= 55:
        composite_signal = "MODERATE_ALPHA_CANDIDATE"
        action = "Mixed factors. Some strengths offset by weaknesses. Selective entry or monitor."
    elif total_score >= 40:
        composite_signal = "WEAK_ALPHA_CANDIDATE"
        action = "Multiple factor headwinds. Underweight or avoid. Look for better opportunities."
    else:
        composite_signal = "AVOID"
        action = "Poor multi-factor profile. Momentum, quality, value, and/or growth all challenged."

    # Factor ranking
    factors_scored = [
        ("momentum", momentum_pts, 25),
        ("quality", quality_pts, 25),
        ("value", value_pts, 25),
        ("growth", growth_pts, 25),
    ]
    factors_scored.sort(key=lambda x: x[1] / x[2], reverse=True)
    dominant_factor = factors_scored[0][0]
    weakest_factor = factors_scored[-1][0]
...
@mcp.tool()
def get_sector_fundamental_heatmap() -> dict:
    """
    11-sector fundamental heatmap: median PE, PB, EV/EBITDA, ROE, revenue growth, net margin.
    Ranks sectors by composite attractiveness score for sector rotation decisions.
    Identifies the cheapest/most expensive sectors across valuation + growth + profitability.
    No API key required.
    """
    import yfinance as yf
    from concurrent.futures import ThreadPoolExecutor
    ...
    # Sector universe: 6 reps per sector (more coverage for robust medians)
    SECTOR_UNIVERSE = {
    ...
    def fetch_ticker_metrics(ticker):
        try:
            info = yf.Ticker(ticker).info
            pe = info.get('trailingPE') or info.get('forwardPE')
            pb = info.get('priceToBook')
    ...
    def safe_median(vals):
        ...
        return round(statistics.median(clean), 2) if len(clean) >= 2 else None
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        results = list(ex.map(fetch_ticker_metrics, all_tickers))

    ticker_map = dict(zip(all_tickers, results))

    sector_data = {}
    for sector, tickers in SECTOR_UNIVERSE.items():
        metrics_list = [ticker_map[t] for t in tickers if ticker_map.get(t)]
        if not metrics_list:
            continue
        med_pe       = safe_median([m["pe"] for m in metrics_list])
        med_pb       = safe_median([m["pb"] for m in metrics_list])
        med_ev       = safe_median([m["ev_ebitda"] for m in metrics_list])
        med_roe      = safe_median([m["roe"] for m in metrics_list])
        med_rev_grw  = safe_median([m["rev_growth"] for m in metrics_list])
        med_margin   = safe_median([m["net_margin"] for m in metrics_list])
        sector_data[sector] = {
            "median_pe":           med_pe,
            "median_pb":           med_pb,
            "median_ev_ebitda":    med_ev,
            "median_roe_pct":      med_roe,
            "median_rev_growth_pct": med_rev_grw,
            "median_net_margin_pct": med_margin,
            "ticker_count":        len(metrics_list),
        }

    # Score each sector: valuation (lower=better), growth+profitability (higher=better)
    # Normalize each metric across sectors and produce attractiveness score 0-100
    def rank_score(val, all_vals, lower_is_better=True):
        """Return 0-10 score based on cross-sector rank"""
        ...
        n = len(clean)
        # Best rank = 10 pts, worst = 0 pts
        return round(10 * (1 - rank / max(n - 1, 1)), 1)

    pe_vals       = {s: d["median_pe"] for s, d in sector_data.items()}
    pb_vals       = {s: d["median_pb"] for s, d in sector_data.items()}
    ev_vals       = {s: d["median_ev_ebitda"] for s, d in sector_data.items()}
    roe_vals      = {s: d["median_roe_pct"] for s, d in sector_data.items()}
    rev_grw_vals  = {s: d["median_rev_growth_pct"] for s, d in sector_data.items()}
    margin_vals   = {s: d["median_net_margin_pct"] for s, d in sector_data.items()}

    scored_sectors = {}
    for sector, d in sector_data.items():
        val_score  = (rank_score(d["median_pe"], pe_vals, True) * 0.4
                    + rank_score(d["median_pb"], pb_vals, True) * 0.3
                    + rank_score(d["median_ev_ebitda"], ev_vals, True) * 0.3)
        qual_score = (rank_score(d["median_roe_pct"], roe_vals, False) * 0.4
                    + rank_score(d["median_net_margin_pct"], margin_vals, False) * 0.3
                    + rank_score(d["median_rev_growth_pct"], rev_grw_vals, False) * 0.3)
        # Composite: valuation 45%, quality+growth 55%
        composite = round(val_score * 4.5 + qual_score * 5.5, 1)

        if composite >= 75: attractiveness = "HIGHLY_ATTRACTIVE"
        elif composite >= 58: attractiveness = "ATTRACTIVE"
        elif composite >= 42: attractiveness = "NEUTRAL"
        elif composite >= 25: attractiveness = "UNATTRACTIVE"
        else: attractiveness = "AVOID_SECTOR"

        scored_sectors[sector] = {
            **d,
            "valuation_score":        round(val_score, 1),
            "quality_growth_score":   round(qual_score, 1),
            "composite_score":        composite,
            "attractiveness":         attractiveness,
        }

    ranked = sorted(scored_sectors.items(), key=lambda x: x[1]["composite_score"], reverse=True)
    top_3    = [s for s, _ in ranked[:3]]
    bottom_3 = [s for s, _ in ranked[-3:]]

    # Best value sector: lowest median PE
    best_value = min(((s, d["median_pe"]) for s, d in sector_data.items() if d["median_pe"]), key=lambda x: x[1], default=(None, None))[0]
    # Best growth sector: highest rev growth
    best_growth = max(((s, d["median_rev_growth_pct"]) for s, d in sector_data.items() if d["median_rev_growth_pct"]), key=lambda x: x[1], default=(None, None))[0]
    ...
    return {
        ...
        "most_attractive_sectors":   top_3,
        "least_attractive_sectors":  bottom_3,
        "highlights": {
            "best_value_sector":        best_value,
            "best_growth_sector":       best_growth,
            "best_profitability_sector": best_roe,
        },
        "strategy_guide": (
            f"OVERWEIGHT: {', '.join(top_3)} — highest composite attractiveness (valuation + quality + growth). "
            f"UNDERWEIGHT: {', '.join(bottom_3)} — least attractive relative to peers. "
            "Rotate toward sectors with HIGHLY_ATTRACTIVE or ATTRACTIVE ratings. "
            "Use get_peer_comparison() for individual stock selection within favored sectors."
        ),
        "source": "Yahoo Finance — no API key required",
        "note": "Medians derived from 6 representative S&P 500 stocks per sector. Refresh weekly for rotation signals."
    }


@mcp.tool()
def get_technical_strength_score(ticker: str) -> dict:
    """
    Comprehensive technical analysis scorecard for a stock.
    Scores trend (MA alignment), momentum (RSI, MACD), Bollinger Band position,
    volume confirmation, and 52-week range position.
    Returns technical strength 0-100 with actionable signals.
    No API key required.
    """
    import yfinance as yf
    import math
    ...
    try:
        ...
        if hist.empty or len(hist) < 50:
            return {"error": "Insufficient price history", "ticker": ticker}
        ...
        # ── Factor 1: Trend (MA alignment) — 30 pts ──
        ...
        # MA alignment: MA20 > MA50 > MA200 = perfect uptrend
        ma_aligned_bull = (ma20 > ma50) and (ma200 is None or ma50 > ma200)
        ma_aligned_bear = (ma20 < ma50) and (ma200 is None or ma50 < ma200)

        if above_ma20 and above_ma50 and above_ma200:
            trend_pts += 20; trend_signals.append("PRICE_ABOVE_ALL_MAS")
        elif above_ma20 and above_ma50:
            trend_pts += 15; trend_signals.append("PRICE_ABOVE_MA20_MA50")
        elif above_ma50:
            trend_pts += 8; trend_signals.append("PRICE_ABOVE_MA50_ONLY")
        elif above_ma20:
            trend_pts += 5; trend_signals.append("PRICE_ABOVE_MA20_ONLY")
        else:
            trend_pts += 0; trend_signals.append("PRICE_BELOW_KEY_MAS")

        if ma_aligned_bull:
            trend_pts += 10; trend_signals.append("MAS_BULLISH_ALIGNMENT")
        elif ma_aligned_bear:
            trend_pts += 0; trend_signals.append("MAS_BEARISH_ALIGNMENT")
        else:
            trend_pts += 5; trend_signals.append("MAS_MIXED_ALIGNMENT")

        trend_detail = {
            "score": trend_pts, "max": 30,
            "current_price": round(current, 2),
            "ma20": round(ma20, 2), "ma50": round(ma50, 2),
            "ma200": round(ma200, 2) if ma200 is not None else None,
            "above_ma20": above_ma20, "above_ma50": above_ma50, "above_ma200": above_ma200,
            "ma_alignment": "BULLISH" if ma_aligned_bull else ("BEARISH" if ma_aligned_bear else "MIXED"),
            "signals": trend_signals,
        }

        # ── Factor 2: Momentum (RSI + MACD) — 25 pts ──
        # RSI (14-period)
        delta = close.diff()
        ...
        hist_current = macd_hist.iloc[-1]
        hist_prev    = macd_hist.iloc[-2] if len(macd_hist) >= 2 else 0
        ...
        # RSI score (15 pts)
        if 50 <= rsi <= 70:
            mom_pts += 15; rsi_sig = "RSI_BULLISH_ZONE"
        elif 40 <= rsi < 50:
            mom_pts += 8; rsi_sig = "RSI_NEUTRAL_LOW"
        elif rsi > 70:
            mom_pts += 10; rsi_sig = "RSI_OVERBOUGHT"  # strong but caution
        elif 30 <= rsi < 40:
            mom_pts += 5; rsi_sig = "RSI_APPROACHING_OVERSOLD"
        else:
            mom_pts += 12; rsi_sig = "RSI_OVERSOLD_REVERSAL_WATCH"  # extreme may bounce
        mom_signals.append(rsi_sig)

        # MACD score (10 pts)
        if macd_current > signal_current and hist_current > 0:
            mom_pts += 10; macd_sig = "MACD_BULLISH_CROSSOVER"
        elif macd_current > signal_current:
            mom_pts += 7; macd_sig = "MACD_BULLISH_DIVERGENCE"
        elif macd_current > 0:
            mom_pts += 5; macd_sig = "MACD_POSITIVE_TERRITORY"
        elif hist_current > hist_prev:
            mom_pts += 3; macd_sig = "MACD_MOMENTUM_IMPROVING"
        else:
            mom_pts += 0; macd_sig = "MACD_BEARISH"
        mom_signals.append(macd_sig)

        mom_detail = {
            "score": mom_pts, "max": 25,
            "rsi_14": round(rsi, 1),
            "rsi_signal": rsi_sig,
        ...
        # ── Factor 3: Bollinger Bands — 15 pts ──
        bb_mid  = close.rolling(20).mean()
        bb_std  = close.rolling(20).std()
        bb_upper = (bb_mid + 2 * bb_std).iloc[-1]
        bb_lower = (bb_mid - 2 * bb_std).iloc[-1]
        bb_mid_val = bb_mid.iloc[-1]
        bb_pct = (current - bb_lower) / (bb_upper - bb_lower) if bb_upper != bb_lower else 0.5  # 0=lower, 1=upper

        bb_pts = 0
        if 0.4 <= bb_pct <= 0.75:
            bb_pts = 15; bb_sig = "PRICE_IN_BULLISH_BB_ZONE"
        elif 0.75 < bb_pct <= 1.0:
            bb_pts = 8; bb_sig = "NEAR_UPPER_BAND_OVERBOUGHT"
        elif 0.2 <= bb_pct < 0.4:
            bb_pts = 8; bb_sig = "NEAR_MID_BAND_NEUTRAL"
        elif bb_pct > 1.0:
            bb_pts = 5; bb_sig = "ABOVE_UPPER_BAND_EXTREME"
        elif bb_pct < 0.2:
            bb_pts = 10; bb_sig = "NEAR_LOWER_BAND_OVERSOLD_WATCH"
        else:
            bb_pts = 5; bb_sig = "BB_BELOW_MID_BEARISH"

        bb_detail = {
            "score": bb_pts, "max": 15,
            "upper_band": round(bb_upper, 2),
            "middle_band": round(bb_mid_val, 2),
        ...
        # Price trend + volume direction
        price_up_5d = close.iloc[-1] > close.iloc[-5] if len(close) >= 5 else False
        vol_5d_avg  = volume.iloc[-5:].mean()
        vol_prev_avg = volume.iloc[-15:-5].mean() if len(volume) >= 15 else avg_vol_20
        vol_trend_up = vol_5d_avg > vol_prev_avg

        vol_pts = 0
        if price_up_5d and vol_trend_up:
            vol_pts = 15; vol_sig = "BULLISH_VOLUME_CONFIRMATION"
        elif price_up_5d and not vol_trend_up:
            vol_pts = 8; vol_sig = "PRICE_UP_LOW_VOLUME_WEAK"
        elif not price_up_5d and vol_trend_up:
            vol_pts = 3; vol_sig = "HIGH_VOLUME_DISTRIBUTION"
        else:
            vol_pts = 6; vol_sig = "NEUTRAL_VOLUME"

        vol_detail = {
            "score": vol_pts, "max": 15,
            "avg_volume_20d": int(avg_vol_20),
            "last_volume": int(last_vol),
            "volume_ratio_vs_20d": round(vol_ratio, 2),
            "volume_trend": "RISING" if vol_trend_up else "FALLING",
            "signal": vol_sig,
        }

        # ── Factor 5: 52-Week Range Position — 15 pts ──
        range_pct = (current - low_52) / (high_52 - low_52) if high_52 != low_52 else 0.5
        pct_from_high = (current / high_52 - 1) * 100

        if range_pct >= 0.75:
            range_pts = 15; range_sig = "NEAR_52W_HIGH_MOMENTUM"
        elif range_pct >= 0.55:
            range_pts = 12; range_sig = "UPPER_HALF_RANGE"
        elif range_pct >= 0.40:
            range_pts = 8; range_sig = "MID_RANGE"
        elif range_pct >= 0.25:
            range_pts = 5; range_sig = "LOWER_HALF_RANGE"
        else:
            range_pts = 2; range_sig = "NEAR_52W_LOW_WEAKNESS"

        range_detail = {
            "score": range_pts, "max": 15,
            "52w_high": round(high_52, 2),
            "52w_low":  round(low_52, 2),
            "range_pct_b": round(range_pct, 3),
            "pct_from_52w_high": round(pct_from_high, 1),
            "signal": range_sig,
        }

        # ── Composite ──
        total = trend_pts + mom_pts + bb_pts + vol_pts + range_pts

        if total >= 82:
            composite_sig = "STRONG_BUY_TECHNICAL"
            action = "Strong technical setup across all dimensions. Trend, momentum, and volume aligned bullish."
        elif total >= 65:
            composite_sig = "BULLISH_TECHNICAL"
            action = "Solid technical structure. Most indicators positive. Consider entry on pullbacks to support."
        elif total >= 50:
            composite_sig = "NEUTRAL_TECHNICAL"
            action = "Mixed technical signals. Wait for clearer trend confirmation before taking large positions."
        elif total >= 35:
            composite_sig = "BEARISH_TECHNICAL"
            action = "Multiple technical weaknesses. Avoid new longs. Consider reducing existing exposure."
        else:
            composite_sig = "STRONG_SELL_TECHNICAL"
            action = "Technical structure broken across multiple dimensions. Strong avoidance signal."
...
@mcp.tool()
def get_price_target_tracker(ticker: str) -> dict:
    ...
    try:
        ...
        n_analysts = info.get('numberOfAnalystOpinions') or info.get('recommendationKey') and 0
        ...
        # Analyst upgrades/downgrades for 30d vs 90d trend
        ...
        # Target range width as confidence proxy
        ...
        return {
            ...
            "upside_signal": upside_sig,
            "action_guide": upside_action,
            "analyst_consensus": analyst_consensus,
            "target_range_width_pct": range_width_pct,
            "recommendation": {
                "label": rec_label,
                "mean_score": round(rec, 2) if rec else None,
                "n_analysts": info.get('numberOfAnalystOpinions')
            },
            "target_trend": target_trend,
            "upgrade_activity": {
                "last_30d": {"upgrades": upgrades_30, "downgrades": downgrades_30, "upgrade_ratio": upgrade_ratio_30},
                "last_90d": {"upgrades": upgrades_90, "downgrades": downgrades_90, "upgrade_ratio": upgrade_ratio_90},
            },
            "recent_analyst_actions": recent_actions,
            "source": "Yahoo Finance — no API key required",
...
@mcp.tool()
def get_sector_momentum_vs_spy(period_days: int = 63) -> dict:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=12) as ex:
            results = dict(ex.map(lambda s: fetch(s), tickers))
        ...
        # Short-term half period for trend acceleration
        ...
        for etf, name in SECTORS.items():
            ...
            # Acceleration: recent half RS vs full RS implies direction
            ...
            sector_data.append({
                ...
                "sector": name,
                "return_pct": ret_full,
                "return_half_period_pct": ret_half,
                "vs_spy_rs": rs,
                "vs_spy_rs_half": rs_half,
                "signal": sig,
                "acceleration": accel,
            })
        ...
        cyclical_rs  = sum(x["vs_spy_rs"] for x in sector_data if x["etf"] in ["XLK","XLY","XLC","XLF","XLE","XLI","XLB"]) / 7
        defensive_rs = sum(x["vs_spy_rs"] for x in sector_data if x["etf"] in ["XLV","XLP","XLU","XLRE"]) / 4
        ...
        return {
            ...
            "spy_half_period_return_pct": spy_ret_h,
...
@mcp.tool()
def get_short_interest_trend(ticker: str) -> dict:
    """
    ...
    try:
        ...
        # Short float label
        spf = short_pct * 100 if short_pct < 1 else short_pct  # normalize to %
        if spf >= 30:
        ...
        # MoM trend label
        if mom_change >= 15:
            mom_label = "STRONG_SHORT_BUILDING"
            mom_score = 0
        elif mom_change >= 5:
            mom_label = "SHORT_BUILDING"
            mom_score = 3
        elif mom_change >= -5:
            mom_label = "STABLE_SHORT_INTEREST"
            mom_score = 8
        elif mom_change >= -15:
            mom_label = "SHORT_COVERING"
            mom_score = 12
        else:
            mom_label = "STRONG_SHORT_COVERING"
            mom_score = 15
        ...
        # Beta: 10pts (high beta = higher squeeze potential)
        beta_pts = min(10, int(beta * 4))

        squeeze_score = sf_pts + dtc_pts + mom_score + beta_pts

        if squeeze_score >= 75:
            squeeze_risk = "EXTREME_SQUEEZE_RISK"
        elif squeeze_score >= 55:
            squeeze_risk = "HIGH_SQUEEZE_RISK"
        elif squeeze_score >= 35:
            squeeze_risk = "MODERATE_SQUEEZE_RISK"
        else:
            squeeze_risk = "LOW_SQUEEZE_RISK"

        strategy = (
            f"Short float {spf:.1f}% ({short_float_label}), DTC {dtc:.1f}d ({dtc_label}). "
            f"Squeeze score {squeeze_score}/100 → {squeeze_risk}. "
            f"MoM change {mom_change:+.1f}% ({mom_label}). "
        )
        if squeeze_score >= 55:
            strategy += "HIGH SQUEEZE POTENTIAL: Watch for volume surge + price breakout as catalyst."
        elif squeeze_score >= 35:
            strategy += "MODERATE SQUEEZE: Monitor for positive catalyst. Short covering may provide tailwind."
        else:
            strategy += "LOW SQUEEZE RISK: Short sellers have minimal pressure to cover."
        ...
        return {
            ...
            "squeeze_risk": squeeze_risk,
            "score_breakdown": {
                "short_float_pts": sf_pts,
                "dtc_pts": dtc_pts,
                "mom_trend_pts": mom_score,
                "beta_pts": beta_pts,
                "max_possible": 100
            },
...
@mcp.tool()
def get_options_max_pain(ticker: str) -> dict:
    """
    Calculate options max pain for a stock: the strike price where option sellers
    (market makers) experience minimum total dollar pain at expiration.
    Analyzes up to 3 nearest expirations within 45 DTE.
    No API key required.
    """
    try:
        import yfinance as yf
        ...
        if not valid_exps:
            return {"error": f"No expirations found within 45 DTE. Nearest: {expirations[0]}"}
        ...
        for exp, dte in valid_exps:
            ...
            # Filter strikes within ±30% of current price
            ...
            for strike in all_strikes:
                # Call pain: ITM calls (strike < current) → holders profit, sellers lose
                call_pain = 0
                for _, row in calls.iterrows():
                    if row["strike"] < strike:  # ITM at this strike level
                        call_pain += (strike - row["strike"]) * row["openInterest"] * 100

                # Put pain: ITM puts (strike > current) → holders profit, sellers lose
                put_pain = 0
                for _, row in puts.iterrows():
                    if row["strike"] > strike:  # ITM at this strike level
            ...
            min_pain_val = pain_by_strike[max_pain_strike]

            # Lowest pain strikes top-5
            sorted_strikes = sorted(pain_by_strike.items(), key=lambda x: x[1])
            top5_low_pain = [{"strike": s, "total_pain_usd": round(v, 0)} for s, v in sorted_strikes[:5]]

            # Distance from current
            dist_pct = ((max_pain_strike - current_price) / current_price) * 100
            if dist_pct >= 3:
                pull_signal = "MAX_PAIN_ABOVE_CURRENT_BULLISH_PULL"
            elif dist_pct <= -3:
                pull_signal = "MAX_PAIN_BELOW_CURRENT_BEARISH_PULL"
            else:
                pull_signal = "NEAR_MAX_PAIN_SIDEWAYS_PRESSURE"
            ...
            if abs_dist <= 2:
                ...
                pin_risk = "MODERATE_PIN_RISK"
            else:
                pin_risk = "LOW_PIN_RISK"
            ...
            results.append({
                ...
                "pin_risk": pin_risk,
                "total_pain_at_max_pain_usd": round(min_pain_val, 0),
                "total_call_oi": total_call_oi,
                "total_put_oi": total_put_oi,
                "pc_oi_ratio": pc_oi_ratio,
                "lowest_pain_strikes_top5": top5_low_pain
            })

        if not results:
            return {"error": "Could not calculate max pain for any expiration"}

        primary = results[0]
        strategy = (
            f"Max pain ${primary['max_pain_strike']} vs current ${primary['current_price']} "
            f"({primary['distance_from_current_pct']:+.1f}%) → {primary['pull_signal']}. "
            f"Pin risk: {primary['pin_risk']}. P/C OI ratio: {primary['pc_oi_ratio']}. "
            "Max pain acts as gravitational pull into expiration. "
        )
        if primary["pin_risk"] == "HIGH_PIN_RISK":
            strategy += f"HIGH PIN RISK: Price likely to converge to ${primary['max_pain_strike']} near expiry. Iron condor centered here."
        else:
            strategy += "Monitor for price drift toward max pain in final week before expiration."

        return {
            "ticker": ticker.upper(),
            "current_price": round(current_price, 2),
            "expirations_analyzed": len(results),
            "primary_expiration": primary,
            "all_expirations": results,
            "strategy_guide": strategy,
            "source": "Yahoo Finance — no API key required",
            "note": "Max pain is the strike where option writers (market makers) have minimum total dollar obligation at expiration."
        }
    except Exception as e:
        return {"error": str(e)}
