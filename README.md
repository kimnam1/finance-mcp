...
Restart Claude. Most tools work immediately — no API keys required.

> **Optional:** Set `FRED_API_KEY` environment variable for full macroeconomic data (free key at [fred.stlouisfed.org](https://fred.stlouisfed.org/docs/api/api_key.html)).
...
- **v4.7.0**: Short interest trend analysis (short % of float HEAVILY_SHORTED ≥30%/HIGH≥15%/MODERATE≥5%/LOW≥2%/MINIMAL, MoM change % STRONG_SHORT_BUILDING/BUILDING/STABLE/COVERING/STRONG_COVERING, days-to-cover EXTREME/HIGH/MODERATE/LOW, composite squeeze score 0-100 from short float 40pts+DTC 35pts+MoM 15pts+beta 10pts → EXTREME/HIGH/MODERATE/LOW_SQUEEZE_RISK; advanced tier) + options max pain calculator (each candidate strike total dollar pain = ITM call losses + ITM put losses × OI × 100, max pain = minimum pain strike, up to 3 expirations ≤45 DTE ±30% spot range, distance signal MAX_PAIN_ABOVE_CURRENT_BULLISH_PULL/BELOW_CURRENT_BEARISH_PULL/NEAR_MAX_PAIN_SIDEWAYS_PRESSURE, pin risk HIGH≤2%/MODERATE≤5%/LOW, total call/put OI + P/C OI ratio, lowest-pain top-5 strikes; advanced tier) — 119 tools
